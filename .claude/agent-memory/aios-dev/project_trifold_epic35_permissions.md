...
**Sub-module support in `canAccess` (Story 35-7, 2026-05-20):** `canAccess` now detects dot-keys like `"configuracoes.clientes"`. For sub-modules it does a **direct query** to `user_permission_exceptions` via `createAdminClient()` (NOT via `getUserExceptions` from `permissions-exceptions-actions.ts` — that would create a circular import because the actions file imports from `permissions.ts`). If no explicit exception found, recurses to `canAccess(userId, orgId, parentModule)` to inherit. Module top-level path (no dot) is unchanged. `SUBMODULE_MAP` constant in `permissions-modules.ts` declares which top-level modules have sub-modules; currently only `configuracoes` (8 sub-modules: clientes, usuarios, empresa, horario, integracoes, personalidade, pipeline, perfil-acesso). The `UserEditModal` exceptions tab renders an expand chevron for parent modules in `SUBMODULE_MAP` and shows indented sub-rows when expanded; `"↳ Herdado do módulo"` is the inherited-state label (vs `"Herdado do perfil"` for top-level). Page `perfil-acesso/page.tsx` no longer uses hardcoded `user.role !== "admin"` — story 35-7 migrated it to `canAccess(user.id, user.orgId, "configuracoes.perfil-acesso")`, superseding the 35-5 scope-OUT exception. Pages `empresa`, `pipeline`, `horario` migrated from `"sistema"` to `"configuracoes.{name}"` (heritage changed but admin/supervisor outcomes preserved — admin has `configuracoes: true` in fallback). Page `horario/page.tsx` has TWO `canAccess` calls (top-level guard + inline `"use server"` action) — both must be updated together.
