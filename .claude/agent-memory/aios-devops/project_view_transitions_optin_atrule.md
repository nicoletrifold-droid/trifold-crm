...
**Escopo do opt-in é por par de documentos:** no controle, o destino (`Empreendimentos.dc.html`) *tem* a at-rule e ainda assim não houve transição, porque o documento de **saída** não tinha. Ou seja, ao adicionar uma página nova ao site é obrigatório colocar a at-rule nela, senão ela quebra a transição em toda navegação que sai dela. `Logo.dc.html` está nesse estado hoje (sem opt-in) — é intencional apenas por ser página utilitária, mas serve como controle de teste. Reconferido em 2026-08-26 nos arquivos versionados: at-rule presente nas 7 `.dc.html`, ausente só em `Logo.dc.html`, e zero ocorrências da meta tag.
