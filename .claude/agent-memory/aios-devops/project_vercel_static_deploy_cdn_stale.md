...
description: Após deploy do trifold-design-system, arquivos removidos podem responder 200 (cache de edge) por alguns segundos — revalidar antes de concluir falha; para asset trocado no mesmo path use SHA256
metadata:
  type: project
---
...
**Ausência de arquivo (não é o mesmo que 404 no edge):** para provar o que de fato subiu no deployment, sem depender de cache nem de roteamento, use `GET https://api.vercel.com/v7/deployments/{dpl_id}/files?teamId={team}&base=/src` — ver [[vercelignore-static-projects-exposure]].
