...
**PENDENCIA FECHADA (2026-08-26 ~17:40Z) — `META_CAPI_TEST_EVENT_CODE` REMOVIDA de prod.** Estava ligada no `trifold-crm` (target production) com o codigo real `TEST15571` (dataset `1337310707164669`, conta "TRIFOLD - VIND") para fechar o T12/AC11 da Story 86-11. Removida com `vercel env rm META_CAPI_TEST_EVENT_CODE production --yes`, ausencia reconferida com `vercel env ls production` (restam so `META_CAPI_DATASET_ID` e `META_CAPI_ACCESS_TOKEN`), e prod redeployada sem ela (`trifold-88krb748r`, aliasada em `crm.trifold.eng.br`). Eventos CAPI de prod voltaram a contar para reporting/otimizacao.
**Why:** enquanto ela existe, TODO evento CAPI de prod vai marcado como teste no Meta e o Events Manager nao contabiliza/otimiza sobre esses eventos. Nao e flag inofensivo.
**How to apply:** se essa env reaparecer em `vercel env ls production`, tratar como esquecida de uma sessao de validacao e perguntar antes de qualquer analise de volume de eventos. `vercel env rm` funciona (nao tem o bug do `add` via stdin), mas exige `--yes` em modo nao-interativo. Sempre redeployar depois — mudanca de env so vale no proximo deploy.

**`vercel redeploy` NAO aceita `--yes`** (falha com "unknown or unexpected option"). Os flags reais sao `--target <env>` e `--no-wait`; em modo agente ele ja roda non-interactive. Uso validado: `vercel redeploy <url-do-deploy-prod> --target production --non-interactive` (~3min, sai "Aliased https://crm.trifold.eng.br").
