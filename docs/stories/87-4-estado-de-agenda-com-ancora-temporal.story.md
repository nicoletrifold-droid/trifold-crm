...
> ### ✅ RATIFICAÇÃO DO @po (08/08) — fecha o **`D4`** do gate `87.4`
>
> **A AC3 estava escrita sem qualificação e o código entregue está CERTO.** Texto qualificado agora,
> pelo @po, que é quem pode editar AC. **A implementação não divergiu da story — a story é que não
> tinha acompanhado.**
>
> **A qualificação:** a guarda de período vale para `fonte: "mencao"` e **não** para
> `fonte: "pendencia"`. **A razão é semântica, não de implementação** — e está no próprio código
> (`pipeline.ts:745-752`, conferido por mim no `HEAD`):
>
> | fonte | o que é | o dia herdado entra quando o lead dá só o período? |
> |---|---|---|
> | `mencao` | texto solto do lead (*"pode ser sábado"*), que **nós não perguntamos** | ❌ **não** — é a guarda desta AC |
> | `pendencia` | dia que ficou pendente **porque NÓS perguntamos** e o lead respondeu | ✅ **sim** — perguntar de novo seria repetir a pergunta que já foi respondida |
>
> ```ts
> // pipeline.ts:750-752 — a distinção, no código entregue
> const hasPendingSlot = isPendencia(agendaState) &&
>   (agendaState!.data_absoluta !== null || agendaState!.hora !== null)
> ```
>
> **Vale com a ressalva `D1` do gate, e ela é parte da ratificação:** a guarda sobrevive **um turno**
> — um turno neutro promove a menção a pendência, e a partir daí o dia entra. **Não é regressão** (o
> `HEAD` fazia igual, e o dia sempre veio de fala do LEAD, porque `extractCollectedData` só escreve
> com `origem: "lead"`), mas **limita o que esta AC entrega** e fica escrito aqui, não só no gate.
> **É o comportamento a vigiar na janela de 24 h.**

Com `agenda_state.data_absoluta` num sábado, **`fonte: "mencao"`**, e a mensagem do lead sendo
*"Semana de manhã"*, o dia herdado **não** entra e o sistema **pergunta o dia**. Com
**`fonte: "pendencia"`** o dia herdado **entra** — e isso é o comportamento correto.
...
| 2026-08-08 | **0.4** | **@po — ratificação escrita da AC3, fechando o `D4` do gate `87.4` (severity LOW, ação requerida "antes do deploy, não é do @dev").** O gate registrou que *"a AC3 manda, sem qualificar, que o dia herdado não entre quando o lead dá só o período; o entregue aplica isso a `fonte: \"mencao\"` e NÃO a `fonte: \"pendencia\"` — e a evidência da conversa real diz que o entregue está certo e a AC está errada"*. **Ratifico: o código está certo, a AC não tinha acompanhado, e a AC foi qualificada** (só o @po pode editar AC). A distinção é semântica e está no `HEAD` (`pipeline.ts:750-752`, `isPendencia(agendaState) && (data_absoluta !== null || hora !== null)`): `mencao` é texto solto do lead, que **nós não perguntamos** — o dia não entra; `pendencia` é dia que ficou pendente **porque nós perguntamos** e o lead respondeu — perguntar de novo seria repetir pergunta já respondida. **A ressalva `D1` entra na ratificação e não como nota de rodapé:** a guarda sobrevive **um turno** (turno neutro promove menção a pendência), não é regressão porque o `HEAD` fazia igual e o dia sempre veio de fala do lead (`extractCollectedData` só escreve com `origem: "lead"`), mas **limita o que a AC entrega** e é o comportamento a vigiar na janela de 24 h. **Nenhuma linha de código muda; nenhum escopo reaberto; o gate PASS de 3 rodadas segue válido.** Continuam abertas as outras pendências daquele gate, que não são minhas: responsável nomeado (D7) e os registros do @pm. | @po (Pax) |
