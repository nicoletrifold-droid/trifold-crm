...
| `lint --force` | **rc=0** |
| `type-check --force` | **rc=0** |
| Higiene R1 (8 hex) | **66 ocorrências / 8 valores distintos · 0 NÃO TRIADOS.** Denominador declarado: os **17** caminhos VERSIONADOS da File List (os 3 de `.claude/agent-memory/**` ficam fora deste commit). Triados por enumeração, não por lista: `git cat-file -t` devolve `commit` para **6**; `f2fc9c8f` é prefixo de `shasum` de arquivo versionado; `00000000` é a família de UUID sintética. |
