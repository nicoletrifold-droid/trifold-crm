...
| `f2fc9c8f` | 2 | prefixo **SHA-1** (`shasum` sem flag) de `route.ts` — **arquivo versionado**, não conteúdo de cliente | `git show 5ee0bf2b:packages/web/src/app/api/webhook/whatsapp/route.ts \| shasum -a 1` → `f2fc9c8f…`; idem em `02e2de5e`. **O algoritmo é parte da alegação:** uma varredura por `sha256` não casa nenhum dos dois e faria este valor parecer não reproduzível. |
...
**A quarta linha entrou na v1.10, e desta vez sem o @qa precisar apontar:** ao consertar a leitura de
`conversations` (CR-87-20-3, seção 13) a leitura IRMÃ — `messages`, no **mesmo `Promise.all`** — ficou
com o `error` não lido. Mesmo predicado estreito, mesma classe larga. Declarada com o raio medido e
com o motivo de não ter ido junto.
...
| Higiene R1 (8 hex) | **69 ocorrências / 8 valores distintos · 0 NÃO TRIADOS.** Os 5 já registrados **mais 3 SHAs de commit desta própria branch**, que entraram porque este registro e o parecer do @qa os citam. Triados um a um, não presumidos: `git cat-file -t` devolve `commit` para **6** dos 8; o 7º (`f2fc9c8f`) é prefixo **SHA-1** de `route.ts` em `5ee0bf2b`/`02e2de5e` (recomputável do repo — ver a triagem da seção 9, que agora nomeia o algoritmo); o 8º é a família de UUID sintética. **Nenhum conteúdo de cliente.** |
| Higiene R2 (UUID fora da família sintética) | **0 linhas** |
| Higiene R3 (data-hora com precisão de minuto) | **29 linhas** — idêntico ao registro anterior, nenhuma linha nova |
...
### 12. Correção de RENDERIZAÇÃO — três linhas de tabela truncadas por `|` literal dentro de crase

Achado pelo CodeRabbit e reproduzido pelo @devops. **Não muda código, teste, fixture, AC nem
veredito** — é render, e mesmo assim é o registro se estragando, que é o tema que já mordeu esta
story quatro vezes por outros caminhos.

**O defeito.** O GFM divide a célula em **todo `|` não escapado**, inclusive **dentro de crase**;
o excedente sobre o número de colunas do cabeçalho é **descartado**. Três linhas tinham mais
células que o cabeçalho e renderizavam truncadas, **perdendo a última coluna** (o autor, nas duas
do Change Log; a decisão, na tabela de irmãos):

| linha | tabela | células × cabeçalho | `\|` literal responsável |
|---|---|---|---|
| 585 | Change Log v1.8 | 6 × 4 | `if (error \|\| !data) return []` (2) |
| 586 | Change Log v1.7 | 6 × 4 | `"aplicada" \| "falhou"` (1) **+** `2 failed \| 21 passed (23)` (1) |
| 1268 | irmãos da classe (seção 10) | 5 × 3 | `if (error \|\| !data) return []` (2) |

**Correção factual ao enunciado que recebi**, medida linha a linha: o conjunto de trechos culpados
está certo, mas a **atribuição por linha** não — `2 failed \| 21 passed (23)` está na **586**, junto
com `"aplicada" \| "falhou"`; quem excede na **1268** é o `||` do fail-aberto, o mesmo trecho da 585.
Totais por linha idênticos aos relatados.

**O conserto: escape, e nada além dele.** `|` → `\|` **apenas dentro dos code spans das 3 linhas de
tabela**. O texto continua dizendo a mesma coisa — provado por bytes, não por leitura: desfazendo
`\|` → `|` no arquivo final, as 3 linhas voltam **idênticas ao `HEAD`**, a contagem de linhas do
arquivo não muda e o delta é de **+6 bytes**, um por barra invertida. As mesmas três cadeias
aparecem também em **prosa** (linhas 1173 e 1195) e ali foram **deixadas em paz** de propósito:
fora de tabela, `\|` renderiza a barra invertida.

**A régua, que é o ponto.** Não há `markdownlint` nem `remark-lint` neste repositório (`package.json`
e `node_modules/.bin` varridos, e não existe `.markdownlint*`), então escrevi um contador próprio —
ele reconhece cabeçalho + linha delimitadora, ignora blocos cercados por ``` e conta as células de
**cada linha de cada tabela** dividindo em `|` **não precedido de `\`**, exatamente a regra do GFM.

- **Controle positivo, exigido antes do conserto:** rodada no `HEAD`, a régua acusa **exatamente as
  3 linhas conhecidas** — 585, 586 e 1268 — varrendo **23 tabelas / 148 linhas**. `rc=1`.
- **Depois do conserto:** **0 linhas excedentes** nas mesmas 23 tabelas / 148 linhas. `rc=0`.
- **Controle sintético, para não confiar só nas 3:** num arquivo canário com uma linha defeituosa,
  uma linha limpa e uma linha **já escapada**, a régua acusa **só a defeituosa** — a escapada e a
  limpa passam. A régua morde a classe, não a aparência.

**Varredura dos irmãos, como pedido.** `docs/qa/po-validation-87-20.md` (markdown, **7 tabelas / 25
linhas**): **0 excedentes**. `docs/qa/gates/87.20-….yml`: só leitura — faz `load` com `js-yaml`
**sem erro** (28 chaves de topo), e as **10 tabelas / 45 linhas** embutidas nos block scalars também
dão **0 excedentes**. Nada a consertar lá, e o gate é do @qa de qualquer forma.

#### Achado 1 do CodeRabbit — RECUSADO, com a razão registrada

O CodeRabbit propõe trocar a agregação **fail-closed** de `nicole-health/route.ts:131` por
*last-wins*. **Recusado pelo coordenador, e eu concordo com a razão.** Hoje a sequência
falhou → aplicada **mantém "falhou"**, que é o **lado seguro**: no pior caso chama um humano à toa.
*Last-wins* inverteria a assimetria — a mesma sequência passaria a dizer "contido", **calando**
justamente quando a contenção falhou. Esse é o modo de falha que a story inteira existe para
eliminar (é literalmente o Achado 1 da seção 10 e a QA-87-20-6 da seção 11, de novo, uma camada
acima). **Não alterado.** Se alguém quiser reabrir, o argumento tem de explicar por que o silêncio
no caso perigoso é preferível ao ruído no caso benigno — não é uma questão de estilo de agregação.

#### Imprecisão conhecida do gate, registrada e NÃO consertada (o gate é do @qa)

A linha `higiene.r3` do gate decompõe as 29 linhas como `… + loop-breaker.test 8 + …`. Medido: quem
tem as 8 é **`packages/ai/src/chat/pipeline-loop-breaker.test.ts`**; o
**`packages/ai/src/flows/loop-breaker.test.ts` tem 0**. São dois arquivos diferentes, em diretórios
diferentes, com nomes confundíveis — e o gate cita os dois em outros pontos, o que torna o rótulo
ambíguo, não só curto. **O total (29) e a composição estão certos**: gate 1 + pipeline-loop-breaker.test 8
+ fixture 1 + nicole-health/route.test 13 + webhook `__tests__`/route.test 1 + nicole-health/route.ts 1
+ admin-whatsapp.test 4 = **29**. É rótulo, não contagem. Fica **declarado para o @qa**, sem toque
meu no arquivo dele.
...
| prova | resultado |
|---|---|
| Régua de células — **antes** do conserto | **3 linhas excedentes** (585, 586, 1268) em 23 tabelas / 148 linhas · `rc=1` — controle positivo |
| Régua de células — **depois** | **0 linhas excedentes** em 23 tabelas / 148 linhas · `rc=0` |
| Régua de células — canário sintético | acusa **só** a linha defeituosa; a **já escapada** e a limpa passam |
| `po-validation-87-20.md` | **0 excedentes** (7 tabelas / 25 linhas) |
| `gates/87.20-….yml` | `js-yaml` `load` **sem erro**; **0 excedentes** nas 10 tabelas embutidas — leitura apenas |
| Equivalência do texto | desfazendo `\|` → `\|`, as 3 linhas voltam **byte a byte** ao `HEAD`; nº de linhas do arquivo inalterado; **+6 bytes** |
| Régua pega o autor dela — **vivacidade não encenada** | ao escrever esta própria seção eu introduzi **2** defeitos novos da mesma classe (o cabeçalho desta tabela de culpados e a linha da equivalência, ambos com `\|` mal escapado). A régua acusou os **2** — um como `DELIM 4 vs cabeçalho 5`, outro como `3 × 2 (EXCEDE)` — e só depois de corrigidos o arquivo saiu `rc=0` com **25 tabelas / 158 linhas**. Não foi um controle que eu montei: foi um vermelho que eu não queria. |
| Escopo do diff | **1 arquivo, 3 linhas** (`3 insertions(+), 3 deletions(-)`) — só o `.story.md` |
| Suíte inteira | **289 arquivos · 3.802 passando · 6 expected-fail** (3.808) · `rc=0` — **idêntica** à da v1.8, que é o que uma mudança em `.md` tem de provar |
| `lint --force` | **rc=0** |
| `type-check --force` | **rc=0** |
| Higiene R1 (8 hex) | **66 ocorrências / 8 valores distintos · 0 NÃO TRIADOS.** Denominador declarado: os **17** caminhos VERSIONADOS da File List (os 3 de `.claude/agent-memory/**` ficam fora deste commit). Triados por enumeração, não por lista: `git cat-file -t` devolve `commit` para **6**; `f2fc9c8f` é prefixo **SHA-1** de `route.ts` em `5ee0bf2b`/`02e2de5e`; `00000000` é a família de UUID sintética. |
| Higiene R2 (UUID fora da família sintética) | **0** |
| Higiene R3 (data-hora com precisão de minuto) | **29** pela régua registrada (ISO com minuto) — **idêntico** ao registro anterior. Uma régua mais larga que eu também rodei acha **+3** linhas de *hora de parede com fuso* (`06:05 BRT`/`09:05 UTC` em 2 arquivos e 1 citação no parecer), **todas pré-existentes e todas hora-do-dia sem data**; declaradas, não novas. |
| O que esta passada **adicionou** | 92 linhas. **UUID 0, telefone 0, e-mail 0, data-hora 0** pela régua registrada (ISO com minuto). Tokens de 8 hex nas linhas novas: `ced550b1`, `f2fc9c8f` e `00000000` — os **três já entre os 8 triados**, e entram só porque **este próprio registro os cita** (o primeiro vem da linha da tabela de irmãos que eu escapei; os outros dois, das linhas de higiene acima). **0 valores distintos novos.** Autorreferência declarada, na mesma disciplina da v1.7/v1.8: a régua larga acusa **2** linhas novas de hora de parede — **a linha de R3 desta tabela e esta própria linha meta**, que recita os mesmos valores ao descrevê-la; ambas repetem `06:05 BRT`/`09:05 UTC` já presentes em 2 arquivos versionados. *(A v1.8 dizia **1**: contei a linha descrita e esqueci a que descreve — a régua mordendo o rabo pela quinta vez, corrigido na v1.10.)* Nenhum dado novo entrou na árvore. |

### 13. Correção pós-revisão COMPLETA do CodeRabbit — a leitura que falha **reativava** a Nicole (CR-87-20-3)

**Por que só agora.** As passadas incrementais do CodeRabbit vinham **pulando commits** e publicando
`success` assim mesmo — verde que era *skip*, não aprovação. O @devops forçou uma revisão completa
(base `aa584dfb` → tip `5cebdbd9`, 17 arquivos), e com o denominador maior ela achou isto.

#### O defeito, e ele é pior do que o relatório disse

`route.ts:1109` derivava `contidaPorLoop` de `convRow?.handoff_reason` sem nunca olhar o `error` da
consulta. O PostgREST **não rejeita** — devolve `{ data: null, error }`. Então `convRow` fica
`undefined` e `contidaPorLoop` vira `false` **sozinho**: a Nicole é reativada numa conversa que a
trava havia contido, o contador da janela de 30 min do Sinal A zera, e a **oscilação permanente que
o AC14 existe para matar volta inteira**, em silêncio.

**A medição acrescenta um andar ao achado.** Com a leitura falhando, `handoff_at` some junto —
`resolveTakeoverAnchor(null, null)` devolve `null` e `shouldReactivateAi(null)` é `true`
**por contrato** (63-13: *"nunca houve mensagem de corretor → reassume"*, `broker-takeover-status.ts:90`).
Ou seja, o fail-open não pulava só a guarda do AC14: **reativava incondicionalmente**, atropelando
também a regra temporal de 63-13/63-15. Um handoff de **60 segundos** — corretor no meio do
atendimento — reativava a Nicole se a leitura falhasse. Isso está medido como teste, não como
argumento.

#### O princípio é o mesmo da correção anterior, agora do lado do LEITOR

Na v1.7, `conterLoop` parou de chamar de sucesso o `UPDATE` que não conseguiu escrever — `contencao`
virou campo **obrigatório e discriminante** porque um booleano opcional teria default e o default
seria *"funcionou"*. Aqui é a leitura cometendo o análogo: **não conseguir ler o motivo do handoff
não é o mesmo que ter lido "não foi contida por loop"**. São dois estados, e no estado *"não sei"* a
ação **irreversível** (reativar) não acontece — não reativar custa uma conversa parada até alguém
olhar, e há aviso para isso.

**E o aviso é metade do conserto.** Fail-closed que **cala** troca um defeito por outro: a conversa
fica parada e ninguém sabe por quê. Por isso o caminho grita — `NICOLE_REATIVACAO_ESTADO_DESCONHECIDO`,
nível `error`, com o **motivo do banco** em `metadata.erro`, `await logEventOnce` (o canal
fire-and-forget morre com a lambda; roda dentro do `after()`). Mesma razão pela qual o grito da
contenção que falhou teve de aparecer.

`shouldReactivateAi` e `resolveTakeoverAnchor` **continuam intocadas** — o conserto é no CHAMADOR,
como o AC14 inteiro sempre foi.

#### Carrasco escrito ANTES, e o instrumento que faltava

O fake do webhook só sabia produzir **dois** estados: *"achei"* e *"não achei"* (`data: []`). O
terceiro — `{ data: null, error }` — não existia, e **sem ele nenhum teste desta suíte conseguia
sequer EXPRESSAR o defeito**. Foi acrescentada uma injeção de falha de leitura por **predicado
(tabela + lista literal de colunas)**, e não por nome de tabela seco: `conversations` é lida em
vários pontos do mesmo turno e derrubar todas mediria outra coisa — a rota nem chegaria ao bloco.

**O par com o controle negativo é o que dá dente aos testes.** O primeiro teste do mesmo `describe`
usa **exatamente a mesma fixture** (`broker_reply`, handoff de 30h) e exige que a Nicole **REATIVE**.
O carrasco novo muda **uma** coisa — a leitura falha — e exige o resultado **invertido**. Fechar a
porta para todo mundo mata aquele controle; deixá-la aberta mata este.

**Vermelho inicial medido: 🔴 3 falhando / 39** no arquivo, com `tsc --noEmit -p packages/web` **rc=0
antes** da contagem (nesta story a armadilha do vermelho-por-erro-de-compilação já enganou quatro vezes).

| mutante | o que faz | resultado | kill set |
|---|---|---|---|
| **M-CR3a** (a obrigatória) | volta a tratar erro como *"não contida"* — remove `&& !motivoDesconhecido` | 🔴 **2 / 39** | os 2 testes de *"não reativa"* |
| **M-CR3b** (**controle negativo**) | `motivoDesconhecido = true` sempre — fecha a porta para todo mundo | 🔴 **1 / 39** | o teste **pré-existente** `broker_reply` com 30h **REATIVA** |
| **M-CR3c** | o grito sem guarda invertida (`if (false)`) — fail-closed que cala | 🔴 **1 / 39** | o teste do grito |
| **M-CR3d** | `await logEventOnce` → `void logEventOnce` | 🔴 **1 / 39** | o teste do grito |

**4 mutantes, 4 vermelhos, todos com `tsc --noEmit` rc=0 antes da contagem.** Os kill sets de
**M-CR3a** e **M-CR3b** são **disjuntos** — é isso que prova que o teste distingue *"não sei"* de
*"não é loop"*, em vez de só trancar a porta. E **M-CR3d** vermelho significa que o `await` é
carregado de verdade aqui: não é folga de relógio completando a promise órfã por acidente, que é a
armadilha que a QA-87-20-1 já pagou nesta mesma suíte.

#### Quem LÊ este evento — medido, não presumido

Evento novo não é destinatário. Medi o predicado de **cada** consumidor de `system_events` do cron:

- `coletarLoops` filtra `event_type = 'NICOLE_LOOP_DETECTADO'` — **não casa**.
- O branch de erro da 87-19 filtra `level = 'error'` (**casa**) e depois passa toda `message` por
  `classificarErroIA`, cujo `ASSINATURAS` (`alerts/erro-ia.ts:46-50`) são **8 cadeias de API de IA
  em inglês**; a frase deste evento não contém nenhuma ⇒ `null` ⇒ `if (!tipo) continue`, **descartado**.

**Então: os destinatários deste grito são a tabela `system_events` (consultável) e o log de erro do
Vercel (`logToConsole` em nível `error`) — não o alerta de WhatsApp ao admin.** É exatamente o que
foi pedido ("evento em nível de erro com o motivo do banco"), e é a **mesma situação** de
`NICOLE_LOOP_CONTENCAO_FALHOU`, já declarada em `nicole-health/route.ts:369-370`. Declarado aqui para
não repetir o defeito da QA-87-20-6, em que um grito sem destinatário foi dado por resolvido.

**Detalhe que fecha um risco:** o texto do banco vai em `metadata.erro`, **não** em `message`, e o
classificador só lê `message` — então um erro de Postgres não pode colidir por acidente com uma
assinatura de IA e produzir um `NICOLE_HEALTH_ALERTA` falso.

#### Os outros 5 achados da revisão completa — triados pelo MESMO critério, todos da 2ª classe

Critério aplicado: **conserta o que desfaz uma garantia da story; declara o que degrada um sinal.**
Nenhum dos cinco é da primeira classe. Cada linha traz a **medição**, não a impressão.

*(Além dos cinco, a varredura da CLASSE do meu próprio conserto achou um sexto ponto que a revisão
não listou: a leitura irmã de `messages`, no mesmo `Promise.all`. Está na tabela de irmãos da
seção 10, com o raio medido — declarada, não consertada, e desta vez antes do gate.)*

| achado | medição | decisão |
|---|---|---|
| `docs/qa/87-20-regua-sinais-loop.sql:85` — `pico_a` não exclui `t = ''` | **verdadeiro:** o runtime TEM a guarda (`loop-breaker.ts:176` — `if (alvo === "") return { bloquear: false }`), a régua não. Mas a divergência é **conservadora por construção**: incluir vazios só pode **aumentar** `bloqueadas_sinal_a`, nunca diminuir — e o número publicado é **1** (o incidente). Corrigir só pode levar de 1 para ≤ 1. | **não consertado.** E há uma razão a mais para não mexer: consertar o SQL **sem reexecutá-lo** deixaria no repositório uma consulta que ninguém rodou ao lado de números produzidos por outra. Reexecutar exigiria ler produção (proibido) — o banco de teste não espelha a população. |
| `docs/qa/po-validation-87-20.md:94` — "última migration 248" | **verdadeiro:** `ls supabase/migrations` termina em **247**. | **não consertado** — é o parecer do @po. O fato já está declarado **duas vezes**: na seção 7 desta story ("Nota de fato") e na evidência de AC13 do gate do @qa. Quem corrige o documento é o dono dele. |
| `webhook/__tests__/route.test.ts:60-65` — falta contrato **sem mock** para `processMessageWithMetadata`, `createAnthropicClient` e o import profundo de `LOOP_BOT_HANDOFF_REASON` | **já coberto, por outro instrumento — medido por mutação.** Renomear `processMessageWithMetadata` em `packages/ai/src/chat/index.ts` ⇒ `tsc --noEmit -p packages/web` **rc=1** com `TS2339` em **`route.ts:1254`**. Renomear `LOOP_BOT_HANDOFF_REASON` em `loop-breaker.ts` ⇒ **rc=1** com `TS2305` em **`route.ts:23`** (+2 pontos). Os dois imports são **tipados**. | **não acrescentado.** Um contrato em runtime seria uma segunda cópia de um vermelho que já existe **e que já é régua obrigatória desta story** (`type-check --force` rc=0). Duplicar carrasco não aumenta cobertura; aumenta manutenção. |
| `cron/nicole-health/route.test.ts:72` — o fake **isenta** `level` do filtro | **verdadeiro e medido:** removendo `.eq("level", "error")` de `nicole-health/route.ts`, `tsc` rc=0 e a suíte do cron sai **35 / 35 VERDE**. O buraco é auto-declarado no próprio fake (`if (col === "level") return true`). | **não consertado.** O filtro é da **87-19**, não garantia desta story, e sua remoção degrada a **precisão** do alerta (o classificador passaria a varrer `warn`/`info`), não a contenção. Fechá-lo exige declarar `level` em todas as fixtures do branch de erro da 87-19 dentro de um PR da 87-20. Declarado com o **verde medido**, para o @qa decidir. |
| *(nitpick)* `flows/loop-breaker.test.ts:460` — o teste do denominador não tem vivacidade | **verdadeiro:** `CONV_CONTROLE.slice(0, 2)` são 2 textos **distintos** — nenhuma mutação de `detectarLoopDeConteudo` faz aquela asserção falhar. A 2ª asserção (`LOOP_REPEAT_MAX_SENDS + 1` contra o literal `3`) **tem** dente. E a metade positiva **já é provada no mesmo arquivo**: mutando `bloquear` para `false`, o arquivo sai 🔴 **3 / 55**. | **não acrescentado.** A vivacidade que falta na linha existe em outras 3 do mesmo arquivo; acrescentá-la seria uma 4ª cópia. Declarado porque a leitura isolada daquela linha sugere mais garantia do que ela dá. |
| *(nitpick)* `alerts/admin-whatsapp.ts:113` — normalizar `motivo` antes do parâmetro de template | **hipotético hoje, enumerado:** existem **2** chamadores de `alertarAdminWhatsApp`, ambos em `nicole-health/route.ts` (:306 e :401), e **3** construções possíveis de `motivo` — `MOTIVO_POR_TIPO[tipo]` (4 constantes de uma linha) e as 2 cadeias de loop (`… — ${link}`, com `link` sem espaço). **Nenhuma pode produzir `\n`, `\t` ou 4 espaços seguidos.** | **não consertado.** Normalizar seria mudar o transporte compartilhado com a 87-19 por um modo de falha que **nenhum chamador atual alcança**. Se um `motivo` de texto livre entrar um dia, a normalização entra com ele — e aí terá carrasco. |

#### Duas imprecisões de REGISTRO, corrigidas — e uma delas estava errada no enunciado

1. **Hora de parede: era 1, são 2.** O registro da v1.8 dizia que a régua larga acusa **1** linha nova;
   são **duas** — a linha de R3 daquela tabela **e a própria linha meta que a descreve**, que recita os
   valores ao explicá-los. Corrigido. É a régua mordendo o rabo pela **quinta** vez nesta story, e o
   padrão já é reconhecível: quem escreve sobre o dado escreve o dado.

2. **`f2fc9c8f` — a triagem estava VAGA, não falsa; e a correção proposta é que estaria falsa.**
   Recebi que a varredura de `sha256` de todo `route.ts` em `origin/main..HEAD` e no worktree deu
   **zero casamentos**, e que eu deveria registrar *"hash de um estado de trabalho não commitado, não
   reproduzível pelo histórico"*. **Medi antes de escrever, e não escrevi:**
   `git show 5ee0bf2b:packages/web/src/app/api/webhook/whatsapp/route.ts \| shasum -a 1` devolve
   **`f2fc9c8f858e5f62…`** — e o mesmo em `02e2de5e`. **Os dois commits estão em `origin/main..HEAD`.**
   A varredura por `sha256` não casou porque **`shasum` sem flag é SHA-1**, e era `shasum` sem flag que
   produziu o valor (seção 8). A alegação original — *"prefixo do `shasum` de arquivo versionado,
   recomputável do repo"* — é **verdadeira**; o que faltava nela era **nomear o algoritmo**, sem o qual
   ela é infalsificável e uma varredura no algoritmo errado a contradiz. É isso que foi corrigido, nos
   **3** lugares em que a triagem aparece. Registrar a versão pedida teria posto uma afirmação **falsa**
   no registro para satisfazer um pedido de correção — o oposto do que o pedido queria.

#### Réguas desta passada

| prova | resultado |
|---|---|
| Vermelho do carrasco, **antes** do conserto | 🔴 **3 falhando / 39** em `webhook/__tests__/route.test.ts` · `tsc --noEmit` **rc=0 antes** da contagem |
| Mutação | **4 mutantes, 4 vermelhos**, `tsc` rc=0 em todos; **M-CR3a** e **M-CR3b** com kill sets **disjuntos** |
| Suíte inteira | **289 arquivos · 3.805 passando · 6 expected-fail** (3.811) · `rc=0` — **+3** sobre 3.802, exatamente os 3 testes novos; `xfail` **inalterado em 6** |
| `lint --force` | **rc=0** (0 erros, 30 warnings pré-existentes — mesma contagem) |
| `type-check --force` | **rc=0** |
| `pnpm --filter @trifold/web build` | **rc=0** |
| Árvore após as mutações | os 4 arquivos mutados restaurados de backup; `git diff --stat` confere **2** arquivos de código alterados nesta passada, nenhum outro |
| Régua de células GFM — **controle positivo** | canário sintético com uma linha defeituosa, uma limpa e uma **já escapada**: acusa **só a defeituosa** · `rc=1` |
| Régua de células GFM — **vivacidade no ALVO** | desescapando **1** `\|` de uma célula que esta passada escreveu, a régua acusa `5 x 4 (EXCEDE)` na linha 996 · `rc=1`; restaurado, `cmp` **idêntico** |
| Régua de células GFM — alvo | **0 linhas excedentes** em **28 tabelas / 188 linhas** · `rc=0` (eram 25/158 antes desta seção) |
| **A régua aprovou o vazio — e foi pega** | a 1ª execução da higiene devolveu `0` em **tudo**. Não era limpeza: em `zsh`, parâmetro **não citado não sofre word-splitting**, então `git diff -- $FILES` virou **um** caminho inexistente e o diff saiu **vazio** — entrada vazia produz zero em qualquer contador. Refeita com os 3 caminhos explícitos (**434 linhas de diff, 310 adicionadas**) e com **aborto em entrada vazia** (`rc=2`), que é o que impede a próxima. |
| Higiene R1 (8 hex) | **31 ocorrências / 7 valores distintos · 0 NÃO TRIADOS** (medido DEPOIS de escrita esta tabela: as ocorrências subiram de 23 para 31 ao registrá-las, os **valores distintos não mudaram** — é exatamente por isso que a régua conta valores, não ocorrências)**.** Triados **por enumeração da classe** (todo token de 8 hex das linhas adicionadas), não por casamento contra lista conhecida: `git cat-file -t` devolve `commit` para **5** (`02e2de5e`, `5cebdbd9`, `5ee0bf2b`, `aa584dfb`, `ced550b1` — commits públicos do repo); `f2fc9c8f` é o prefixo **SHA-1** de `route.ts` nos dois primeiros; `00000000` é a família de UUID sintética. **`02e2de5e` é o único valor novo desta passada**, e entra porque a correção da triagem **precisa citar o commit** que a torna reproduzível. Nenhum conteúdo de cliente. |
| Higiene R2 (UUID fora da família sintética) | **0** |
