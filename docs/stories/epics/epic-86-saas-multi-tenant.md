...
updated_at: 2026-08-02  # D11/D12/D13/D14 + correções da validação @po (docs/qa/epic-86-po-validation.md)
created_by: Morgan (@pm)
priority: P0
tipo: Brownfield Enhancement (PRD-level, multi-story, 8 ondas)
numbering_note: "epic 86 — prefixos 84 e 85 pertencem a fixes pontuais não relacionados (ver §0.1)"
objetivo_negocio:
...
stories_planned: [86-1 .. 86-50, exceto 86-45 (removida por D14); 86-27 e 86-42 quebradas em a/b — 51 stories ativas]
open_questions: [PEND-1, PEND-1b, PEND-1c, PEND-4, PEND-6, PEND-8, PEND-9, regra-de-contagem-de-atendimento]
resolved_questions: [PEND-0 (epic 86), PEND-2 → D11, PEND-3 → D14 (não), PEND-5 → D13, PEND-7 → D12]
---

# Epic 86 — Trifold CRM → SaaS Multi-Tenant

## 0. Antes de qualquer coisa: dois avisos operacionais

### 0.1 Numeração — resolvido em 2026-07-30

**Este epic é o 86, não o 84.** Os prefixos 84 e 85 já estavam ocupados por stories pontuais que **não pertencem a este epic** e continuam válidas onde estão:

- `docs/stories/84-1-fix-created-by-auth-id-email.story.md` — fix isolado do `created_by`, sem relação com o pivô SaaS.
- `docs/stories/85-1-fix-subject-ausente-email-templates-api.story.md` — fix isolado de template de e-mail.

Pela convenção do projeto (maior número entre os arquivos de `docs/stories/epics/` **e** os prefixos de story na raiz de `docs/stories/`, +1), o próximo livre era **86**. As stories deste epic são `86-1` … `86-50`, das quais **51 estão ativas** (com `86-27` e `86-42` quebradas em `a`/`b`): o ID `86-45` foi **removido por decisão de produto** (D14, sem impersonation) e deliberadamente **não** foi reaproveitado — ver §3.2. `86-45` não é lacuna nem esquecimento.
...
| **`lib/billing/**` e `platform_services`/`service_billing_*` (Epic 78)** | Domínio diferente: é o custo que a **Trifold paga**. Não ganha `org_id`. Só é tocado em 86-16, para re-ancorar a policy. |
...
| D2 | Gateway de pagamento | Nenhum agora; cobrança fora do sistema, atrás de `BillingProvider` | `ManualBillingProvider` em 86-43; Onda 8 sob demanda |
| D3 | Onboarding | Provisionamento pela Trifold via `/platform`; sem signup público | 86-21, 86-22 |
| D4 | Usuário × org | 1 usuário = 1 empresa; Trifold acessa via `platform_admin` | Fecha Q9/P10; 86-16 |
| D5 | Suspensão por inadimplência | **Manual**, nunca automática | 86-46: cron alerta, não suspende |
| D6 | Ambiente de teste | Supabase descartável **autorizado** (ainda não existe) | PRE-1; 86-3 |
| D7 | Planos | **3 tiers acumulativos**, exatamente a composição sugerida em §11.3 Q8 | 86-27a (composição) + 86-27b (preço) — ver §3.1 |
...
| # | Tema | Decisão | Consequência no epic |
|---|---|---|---|
| D11 | Medição de IA por baixo da venda | **Sim** — vender em atendimento **e** persistir custo real por chamada, internamente | Desbloqueia 86-33, 86-34, 86-35, 86-37; ver §4 |
| D12 | Política no 100% da cota | **`overage`** para `active` (a Nicole nunca para; excedente vai para a fatura); `hard_stop` só em trial; **hard cap de 3×** a cota como proteção contra loop de bug | Desbloqueia 86-38; fecha Q2/Q2 do ADR-007 §4 |
| D13 | Retenção pós-cancelamento | **90 dias de janela de export, depois exclusão definitiva** | Desbloqueia 86-46; exige exclusão real **incluindo Storage** e cláusula de contrato |
| D14 | Impersonation | **NÃO haverá impersonation.** Suporte opera só com metadados e agregados | **Remove a story 86-45**; invalida parcialmente o ADR-004; eleva `86-42b` e PEND-4 (ver §3.2) |
...
- A story `86-45` inteira: `createImpersonationClient`, o overlay em `getServerUser()`, o banner vermelho com cronômetro, `platform_impersonation_sessions`, o e-mail de notificação ao admin do cliente, os 7 controles do ADR-004.
...
4. **`createOrgScopedAdminClient` (86-14) continua obrigatório.** Ele foi projetado como o wrapper reaproveitado pela impersonation, mas sua razão principal sempre foi outra: o piso de isolamento das 166 rotas em service-role (R1). D14 não o afeta.
...
|---|---|
| Camada `platform_admin` (tabela `platform_admins`, níveis `owner`/`operator`/`support`, `requirePlatformAdmin`, auditoria append-only, `withPlatformAdmin` como decorador) | **válida** — implementada em `86-16` e `86-42a` |
...
Custo de implementar: **zero incremental** — e foi o que fechou a decisão. O proxy sobre `createAnthropicClient()` (Onda 4, story 86-34) já lê `response.usage` para contar o atendimento; gravar `cost_micro_usd` na mesma linha é a mesma escrita. As tabelas já preveem as duas colunas (`saas-multi-tenant.md` §2.5).

**Consequência de aceite, não opcional:** `/platform/usage` (86-35) tem de mostrar **margem por org**, não só consumo. Sem essa tela, o custo é coletado e ninguém olha — e a decisão perde a razão de existir.

### 4.2 Como fica modelado

| Dimensão | Unidade | Onde vive | Quem vê |
|---|---|---|---|
| Cota vendida | atendimentos/mês | `plans.ai_quota_atendimentos` e `org_billing_periods.consumed_atendimentos` — **nomes próprios, não `*_credits` reinterpretado** (ver 86-33: no ADR-007 "crédito" significa custo real, e reaproveitar o identificador criaria ambiguidade em cima de uma linha de fatura) | Cliente (`/dashboard/configuracoes/plano`) e Trifold |
...
| FR-28 | `/platform` só consulta a lista fechada `PLATFORM_READABLE_TABLES`, validada em runtime por `platformQuery()` (nasce em `86-22`, endurecida em `86-42a`, lista fechada em `86-42b`) e por teste que varre `app/api/platform/**` | §3.4 |
...
| R6 | Medição de IA quebra a Nicole em produção | média | **crítico** | `ctx` opcional, proxy com `try/catch`, shadow mode, reconciliação de ±5% (86-36) |
| R7 | **Margem negativa invisível** por vender atendimento e não medir custo | **média** | **médio-alto** | **Mitigado por D11:** custo real por chamada em `ai_usage_events` + margem por org em `/platform/usage` (86-35). O risco residual passa a ser não *olhar* a tela — por isso a margem é AC de 86-35, não item de backlog |
...
| R12 | Deriva entre `getHardcodedPermissions()` (TS) e o seed de provisionamento (SQL) | média | médio | `role_default_permissions` como fonte única + teste de paridade (86-21) |
| R13 | Storage: mudar bucket para privado quebra URLs já distribuídas a clientes | **alta** | alto | 86-12 exige varredura das referências gravadas no banco antes do flip; URL assinada com fallback e período de sobreposição |
...
| R15 | Colisão silenciosa de `CREATE OR REPLACE FUNCTION` entre migrations | média | alto | Asserção **R10** do gate (86-2). Quase reverteu a Story 75-226 neste ciclo |
...
| **P5** `privacy_consents` sem `org_id` | 🟡 **só a partir de `86-4`** | R1-R3 avaliam apenas tabelas que **já têm** `org_id`. Enquanto a coluna não existir, o gate é cego para esta tabela **por construção** — não por falha. Depois que `86-4` a adicionar, ela entra no alcance de R1 e R2 e passa a ser protegida contra regressão |
| **P4** tabelas de custo do Epic 78 | ❌ | são tabelas de **plataforma**, legitimamente sem `org_id` — vão para a allowlist. O controle é `86-16`, não o gate |
| **P7** Storage sem escopo de org | ❌ | o gate lê `pg_policies` de `public`; policies de `storage.objects` e buckets públicos estão fora do alcance. Controle: `86-11`/`86-12`/`86-13` |
| **P9** UNIQUEs globais que colidem | ❌ | é forma de constraint, não isolamento. Controle: `86-5` |
| **P10** `users.auth_id UNIQUE` | ❌ | decisão de produto (D4), não defeito |
| **P11** 16 índices faltando | ❌ | performance, não isolamento. Controle: `86-6` |
| **P12** `whatsapp_pricing` global | ❌ | dado global aceito; decisão registrada em `86-16` |
...
**Ressalva obrigatória, escrita no próprio relatório do gate:** este gate valida o **banco**, não o código. Não vê query em service-role sem `.eq("org_id")` — a maior superfície de risco (R1/CON-4). O par indispensável é a regra de ESLint da story 86-14.
...
| 86-44 | são 6 telas distintas — é um mini-épico, não uma story |
| 86-48 | dual-run de roteamento / cutover |
...
| `kanban_stages.semantic_key`, `properties.semantic_key` | `86-19` | `86-20` (resolver, dual-run, cutover, contract) | `86-19` | expand aqui, migrate/contract lá |
| `role_default_permissions` | `86-21` | — | `86-21` | Atômico |
| `org_integrations` | `86-21` (esqueleto) | `86-47` (`secret_ref`/Vault, índices de roteamento reverso, `resolveIntegration`) | `86-21` | **Movido de `86-47` (B2)** — a Onda 7 é sob demanda e pode nunca acontecer |
| `provision_org()` | `86-21` (`p_plan_id` NULL) | `86-31` (`p_plan_id` não-nulo + cria a assinatura) | `86-21` | Fatiado para quebrar o ciclo O2↔O3 (B3) |
| `PLATFORM_READABLE_TABLES` | `86-22` (provisória mínima) | `86-31`, `86-35`, `86-44` (cada tela acrescenta as suas tabelas) · `86-42a` **consolida e audita** · `86-42b` **fecha** 🔒 | `86-22` | **O artefato mais fatiado do epic — 6 stories.** É o que o E1 revelou |
| `platformQuery()` | `86-22` | `86-42a` (endurece + regra R12 do gate) | `86-22` | **Movido de `86-42a` para cá (F3)** — `86-35` o exige na Onda 4 |
| `sellable_modules`, `plans`, `plan_modules`, `plan_limits`, `org_module_grants`, `org_limit_overrides` | `86-26` | `86-27a` (seed do catálogo e composição) · `86-27b` (preços e cotas) 🔒 | `86-26` | DDL atômico; o seed é que é fatiado |
| `org_subscriptions` | `86-26` | `86-31` (cria as linhas), `86-46` (lifecycle de status) | `86-21` (menção **negativa**) | |
| `org_billing_periods` | `86-26` | `86-33` (`cost_micro_usd`), `86-37` (`consumed_atendimentos`), `86-41` (`overage_*`) | `86-21` (menção **negativa**) | **Movido de `86-33` para cá (F1).** Três ondas escrevem colunas nele |
| `ai_usage_events`, `ai_usage_daily`, `record_ai_usage()` | `86-33` | `86-37` (incremento de atendimento dentro da RPC) | `86-33` | |
...
#### 86-2 — Gate de tenancy (R1-R12) com baseline e catraca, não-bloqueante
...
**Dep:** 86-1 · **Est:** G · **Executor:** @dev + @data-engineer
...
**Dep:** 86-2 · **Est:** P · **Executor:** @data-engineer

#### 86-5 — Lote 1 / P9: UNIQUEs compostos com `org_id`
...
**Dep:** 86-2 · **Est:** M · **Executor:** @data-engineer

#### 86-6 — Lote 1 / P11: 16 índices `CONCURRENTLY` iniciando em `org_id`
...
#### 86-7 — P13 + varredura final de views: `search_path` e `security_invoker`
...
#### 86-8 — Policies org-scoped, lote A: leads, conversas, mensagens (PII primeiro)
...
**Dep:** 86-2 · **Est:** G · **Executor:** @data-engineer

#### 86-9 — Policies org-scoped, lote B: agenda, obras, portal do cliente, pastas
**Objetivo:** mesmo padrão do lote A no domínio de obras/agenda/portal.
**AC:**
- **O conjunto de tabelas deste lote é a partição B de `docs/audits/rls-gate-baseline.json`** (agenda, obras, portal, pastas) — enumerada no baseline, não neste epic: a lista só é conhecível depois que 86-2 emitir o arquivo.
...
> **Nota de execução:** draftável agora; **não promover a `Ready`** antes de 86-2 emitir o baseline.
**Dep:** 86-8 (padrão estabelecido) · **Est:** G · **Executor:** @data-engineer

#### 86-10 — Policies org-scoped, lote C: financeiro, imob, campanhas, marketing, `system_events`
...
- Migration inversa documentada.
> **Nota de execução:** draftável agora; **não promover a `Ready`** antes de 86-2 emitir o baseline.
**Dep:** 86-8 · **Est:** G · **Executor:** @data-engineer

#### 86-11 — Lote 2a / P7a: policies de Storage ancoradas em org
...
#### 86-12 — Lote 2b / P7b: buckets com PII → privados + URL assinada
...
**Dep:** 86-11 · **Est:** G · **Executor:** @dev + @data-engineer

#### 86-13 — Lote 2c / P7c: convenção de path `{org_id}/…` + migração de objetos
...
**Dep:** 86-12 · **Est:** G · **Executor:** @dev + @data-engineer

#### 86-14 — Lote 3 / P8: `createOrgScopedAdminClient()` + regra de ESLint
...
- Regra entra como **warn** nesta story; vira **error** ao fim de 86-15.
...
**Dep:** 86-2 · **Est:** M · **Executor:** @dev

#### 86-15 — Migração das rotas service-role que tocam PII para o client escopado
...
**Dep:** 86-14 · **Est:** G · **Executor:** @dev

#### 86-16 — Lote 3 / P4: papel `platform_admin`, trilha imutável + re-ancoragem das tabelas do Epic 78
...
- **Por que a tabela nasce aqui e não em `86-42a`, onde o mecanismo é construído:** ela passa a ser **escrita** já em 86-21 (`action='org.create'`, Onda 2) e 86-31 (Onda 3). Criá-la sem o `REVOKE` e endurecê-la quatro ondas depois deixaria a trilha do provisionamento do **primeiro cliente pagante** forjável e apagável por `service_role` — violando o NFR-13 deste epic por 4 ondas. Imutabilidade é atributo de nascimento, não de refino.
...
- **Esta story precede a criação da primeira conta admin de cliente (86-22/86-25).** Gatilho do dano é a Onda 2, não a Onda 6.
**Dep:** 86-2 · **Est:** M · **Executor:** @dev + @data-engineer

#### 86-17 — Testes de isolamento cross-tenant, data-driven
...
- Roda no CI contra o projeto descartável (86-3), não contra produção.
- Job passa a `continue-on-error: false` ao fim da story.
**Dep:** 86-3, 86-8, 86-9, 86-10 · **Est:** M · **Executor:** @qa + @dev

#### 86-18 — Baseline a zero + gate bloqueante
...
**Dep:** 86-4 … 86-17 (todas) · **Est:** P · **Executor:** @qa + @devops
...
#### 86-19 — Expand: chaves semânticas de stage e imóvel (aditivo, backfill)
...
**Dep:** 86-18 · **Est:** M · **Executor:** @data-engineer

#### 86-20 — Resolver de stage/imóvel + dual-run + cutover + contract (14 arquivos)
...
**Dep:** 86-19 · **Est:** G · **Executor:** @dev

#### 86-21 — `role_default_permissions` + `provision_org()` idempotente
...
- **`p_plan_id` é NULLABLE e na Onda 2 é sempre NULL.** A arquitetura é explícita — §10, Onda 2: *"Mínimo do `/platform`: layout + guard + `/platform/orgs` + `/platform/orgs/new`. **Sem plano, sem fatura.**"* — e a assinatura herdada da §7.4 contradizia isso. **A incoerência é da arquitetura; a resolução é esta:** a Onda 2 provisiona org **sem plano**, sem `org_subscriptions` e sem `org_billing_periods`. Esses passos entram quando `plans` existir (86-26/27a, Onda 3), como AC de 86-31.
- Consequência de ordem, e é a razão de ser desta escolha: sem ela o grafo de artefatos fecha um **ciclo** (86-25 → 86-27 → 86-26 → 86-25) e o critério de saída da Onda 2 passaria a depender de um preço em aberto — a inversão exata que o §1.1 proíbe.
...
- **Esqueleto de `org_integrations` criado nesta story** (tabela + `org_id` + `provider` + `status` + `config jsonb` com identificadores públicos + `UNIQUE (org_id, provider)` + RLS): `provision_org` a semeia, e ela **não pode nascer na Onda 7**, que é declarada sob demanda e pode nunca acontecer. O que fica para 86-47 é o que é de fato da Onda 7: `secret_ref`/Vault, índices UNIQUE de roteamento reverso, `resolveIntegration`, `platform_shared`. É o mesmo expand→migrate que o epic exige de todo o resto (NFR-1).
...
**Dep:** 86-19, 86-16 · **Est:** G · **Executor:** @data-engineer + @dev

#### 86-22 — `/platform` mínimo: layout, guard, lista de orgs, wizard de provisionamento
...
- **O wizard não tem seleção de plano nem de módulos extras nesta onda** — `plans`, `plan_modules` e `org_module_grants` só existem a partir de 86-26 (Onda 3). Os campos comerciais entram em 86-31, que é dona da tela de plano. Antecipá-los aqui criaria dependência de artefato de uma onda futura, que é o defeito que esta correção fecha.
...
- **`platformQuery(table, orgId)` e a constante `PLATFORM_READABLE_TABLES` nascem nesta story**, junto da primeira tela de `/platform`, com a lista provisória mínima (`organizations`, `users`) e o comentário de topo *"lista PROVISÓRIA — consolidada por 86-42a, fechada por 86-42b"*.
- **Regra de crescimento da lista, válida do `86-22` em diante:** *toda story que adiciona tela de `/platform` acrescenta, no mesmo PR, as tabelas que aquela tela lê* — com uma linha de justificativa por tabela. Sem essa regra, a próxima tela bate em `platformQuery()`, é rejeitada, e o @dev contorna o mecanismo exatamente onde ele deveria valer. Quem estende: `86-31`, `86-35`, `86-44`. Quem consolida e audita: `86-42a`. Quem fecha: `86-42b`. Motivo: `86-35` (Onda 4) já exige "só via `platformQuery()`", e `86-42a` é da Onda 6 — declarar a dependência para frente inverteria a ordem de ondas. Vale o mesmo princípio do `platform_audit_log`: **o artefato nasce onde é primeiro usado**, e é refinado depois (`86-42a` adiciona o decorador, os níveis e a regra R12; `86-42b` fecha a lista). Como efeito colateral, `/platform/orgs` deixa de ser a única tela de plataforma sem guarda de leitura.
- **Convite de usuário só pode criar usuário na org de quem convida** (§8.5 da arquitetura). Movido para cá vindo de 86-49 (Onda 7, sob demanda): é vetor de escalada cross-tenant e **a primeira conta admin de cliente nasce nesta story**. Atenuante verificado no código — `packages/web/src/app/api/brokers/route.ts` já deriva `org_id` de `appUser.org_id` em todos os pontos de escrita — então o trabalho é **verificar e replicar**, não corrigir.
**Dep:** 86-16, 86-21 · **Est:** M · **Executor:** @dev

#### 86-23 — 37 crons iteram orgs (`forEachActiveOrg`) — fim do `DEFAULT_ORG_ID`
...
- Ainda **sem** entitlement (só iteração) — o filtro por módulo entra em 86-30.
**Dep:** 86-20 · **Est:** G · **Executor:** @dev

#### 86-24 — Roteamento de webhook por identificador, com dual-run
...
**Dep:** 86-23 · **Est:** M · **Executor:** @dev

#### 86-25 — Org "Trifold Sandbox" + teste de aceitação de multi-tenancy
...
- Org "Trifold Sandbox" criada em produção via `/platform/orgs/new`, **sem plano** (a Onda 2 não tem `plans` — ver 86-21/86-22).
...
- **A AC (b) da §7.4 — "vê Pipeline/Leads/Imóveis e não vê Obras/Lançamentos" — NÃO é verificável nesta onda e foi movida para 86-32.** Motivo, e é factual: `provision_org` semeia `role_permissions` para os **26 `ALL_MODULES`** em cada role; sem a camada de entitlement (86-28) o admin de uma org nova **vê tudo**, inclusive Obras e Lançamentos. Exigir (b) aqui seria uma AC insatisfazível por construção, e ela já está coberta em 86-32 ("Trifold Sandbox com plano `crm` vê exatamente os módulos do tier 10").
- Roteiro do teste versionado e re-executável a cada onda seguinte, ganhando (b) quando a Onda 3 fechar.
**Dep:** 86-22, 86-24 · **Est:** P · **Executor:** @qa
...
#### 86-26 — Modelo de dados de tenancy + entitlement derivado
...
- **`CREATE TABLE org_billing_periods`** (1 linha por org por ciclo: `period_start`/`period_end`, `quota_*`, `alert_80_at`, `alert_100_at`, `closed_at`) — **a tabela nasce aqui, não na Onda 4.** Ela é consumida por `org_entitlement_snapshot()` nesta mesma story ("linha do ciclo") e por `86-31` (Onda 3), duas ondas antes de `86-33`. Mesma classe do defeito do `platform_audit_log`: criar tarde deixa duas stories sem onde ler.
...
**Dep:** 86-25 · **Est:** M · **Executor:** @data-engineer

#### 86-27a — Seed do catálogo de módulos e da composição dos 3 tiers (**sem preço**)
...
- Colunas de preço e cota deixadas **NULL**, com `NOT NULL` adiado para 86-27b — plano sem preço não pode ser vendido, mas pode ser semeado e testado.
**Dep:** 86-26 · **Est:** P · **Executor:** @data-engineer
**Por que existe separada:** enquanto catálogo e preço eram uma story só, PEND-1 travava por transitividade **20 stories** (86-27 → 86-32 → Ondas 4, 5, 6, 7, 8). Separados, um preço em aberto trava apenas a cobrança — que é o que a tabela de PRE-3 sempre dizia e agora é verdade.

#### 86-27b — 🔒 Preços e cotas dos 3 tiers
...
**Dep:** 86-27a · **Est:** P · **Executor:** @pm (valores) + @data-engineer (seed)

#### 86-28 — Camada de entitlement no código + interseção no RBAC
...
**Dep:** 86-26 · **Est:** M em código, **alto em consequência** · **Executor:** @dev

#### 86-29 — `ModuleLockedScreen`, cadeado na sidebar e CTA de upgrade
...
**Dep:** 86-28 · **Est:** M · **Executor:** @ux-design-expert + @dev

#### 86-30 — Entitlement no lado servidor: crons e webhooks
...
**Dep:** 86-28, 86-23 · **Est:** M · **Executor:** @dev

#### 86-31 — `/platform/orgs/[id]/plano` + `/dashboard/configuracoes/plano`
...
- **Estende `PLATFORM_READABLE_TABLES`** (regra de crescimento de `86-22`) com `org_subscriptions`, `plans`, `plan_modules`, `org_module_grants` e `org_limit_overrides` — as tabelas que esta tela lê —, uma justificativa por tabela, no mesmo PR.
...
- **Sem lista de faturas nesta onda**: `tenant_invoices` só nasce em `86-43` (Onda 6). A seção de faturas é adicionada por `86-43`, na mesma story que cria a tabela. É a mesma correção da AC (b) de `86-25` — não exibir na Onda 3 o que só existe na Onda 6.
...
- Toda mutação registra a trilha via `platform_audit()` — **que já existe desde 86-16**, com `REVOKE UPDATE, DELETE` aplicado no nascimento (B1). Nenhuma escrita de auditoria acontece em tabela mutável.
**Dep:** 86-28, 86-22 · **Est:** M · **Executor:** @dev

#### 86-32 — Trifold no plano `completo-interno` + verificação de NFR-2
...
- **Herda a AC (b) de 86-25** (movida por ser insatisfazível na Onda 2): a Sandbox vê Pipeline/Leads/Imóveis e **não** vê Obras/Lançamentos.
**Dep:** **86-27a**, 86-29, 86-30, 86-31 · **Est:** **M** (o checklist das 87 páginas do dashboard não cabe em <1 dia-agente) · **Executor:** @qa
...
#### 86-33 — Modelo de dados de consumo de IA + RPC de registro
...
- **`org_billing_periods` já existe desde `86-26`** (Onda 3). Esta story **adiciona `cost_micro_usd` de forma aditiva** — mesmo padrão que `86-37` usa para `consumed_atendimentos`. Não recriar a tabela.
...
**Dep:** 86-32 · **Est:** M · **Executor:** @data-engineer

#### 86-34 — Proxy de medição na fábrica do client + `ctx` nos 12 call sites
...
**Dep:** 86-33 · **Est:** G · **Executor:** @dev

#### 86-35 — `/platform/orgs/[id]/ia` + `/platform/usage` **com margem por org**
...
- **Estende `PLATFORM_READABLE_TABLES`** com `ai_usage_daily` e `org_billing_periods` (regra de `86-22`).
...
- `/platform/usage` agrega cross-org; só via `platformQuery()` — **que já existe desde `86-22`**, com a lista ampliada aqui para incluir `ai_usage_daily` e `org_billing_periods`.
- Nenhuma PII de lead nessas telas (`PLATFORM_READABLE_TABLES`).
**Dep:** 86-34 · **Est:** M · **Executor:** @dev

#### 86-36 — Reconciliação com o oráculo do Epic 78 (gate de saída da onda)
...
#### 86-37 — Unidade de venda "atendimento": definição contável e contador do ciclo
...
- **Esta story introduz `org_billing_periods.consumed_atendimentos`** (migration aditiva) e o incremento correspondente na RPC `record_ai_usage`, que 86-33 deixou deliberadamente de fora. A UI do cliente exibe a mesma unidade do contrato, sem tradução nem estimativa.
...
**Dep:** 86-36 · **Est:** M · **Executor:** @pm (regra) + @dev

#### 86-38 — `checkAiQuota` + `pickModel(tier, verdict)` + desligamento de flows opcionais
...
**Dep:** 86-37 · **Est:** M · **Executor:** @dev

#### 86-39 — Cron `ai-quota-notify` + barra de consumo no dashboard do cliente
...
**Dep:** 86-38 · **Est:** M · **Executor:** @dev

#### 86-40 — `AI_QUOTA_ENFORCEMENT=on` com rollout escalonado
...
**Dep:** 86-39, 86-36 · **Est:** P · **Executor:** @devops

#### 86-41 — 🔒 Faturamento do excedente de atendimentos
...
- Linha `kind='ai_overage'` gerada por `buildInvoiceForPeriod` (86-43).
...
**Dep:** 86-38 · **Est:** M · **Executor:** @pm (valores) + @dev

---

### Onda 6 — Painel completo, faturamento interno e auditoria (5 stories: 86-42a, 86-42b, 86-43, 86-44, 86-46 — 86-45 removida por D14)
...
#### 86-42a — Mecanismo de plataforma: `withPlatformAdmin`, níveis e `platformQuery()`
...
- **Esta story CONSOLIDA E AUDITA a `PLATFORM_READABLE_TABLES`** — não a cria e não a entrega. A constante nasce em `86-22` e cresce incrementalmente por `86-31`, `86-35` e `86-44`, cada uma acrescentando as tabelas da própria tela (regra de crescimento de `86-22`). Aqui ela é: revisada entrada por entrada, com a justificativa de cada tabela conferida contra o que a tela realmente lê; limpa de entradas órfãs (tabela na lista que nenhuma tela consulta); e congelada contra crescimento silencioso.
- Comentário obrigatório no topo do arquivo: *"lista PROVISÓRIA — consolidada em 86-42a, fechada por 86-42b quando PEND-4 for respondida."*
- **A lista consolidada aqui ainda não é a resposta de PEND-4** — é o retrato auditado do que o produto passou a ler. `86-42b` a confronta com a decisão do dono do produto e a fecha.
- `platformQuery(table, orgId)` — **criada em `86-22`** — é endurecida aqui: rejeição em runtime contra a lista vigente, `select` explícito de colunas, nunca `select('*')` em `users`, e a regra R12 do gate passando a cobri-la.
- Teste que varre `app/api/platform/**` procurando `.from("…")` fora da lista. Regra **R12** do gate ativada.
- `platform_audit_log` e `platform_audit()` **já existem desde 86-16** — esta story consome, não cria.
- **Esta story é draftável e entregável sem a resposta do Gabriel.** O mecanismo depende de *existir uma lista*, não de saber qual é — e o bullet acima garante que existe uma. O que depende da decisão é o **conteúdo** da lista, isolado em `86-42b`.
**Dep:** 86-32 · **Est:** M · **Executor:** @dev

#### 86-42b — 🔒 A lista: o que o super-admin pode ver de um cliente
...
**Dep:** 86-42a · **Est:** P · **Executor:** @pm (decisão) + @dev (implementação)

#### 86-43 — Faturas internas + `ManualBillingProvider`
...
- **Esta story adiciona a seção de faturas a `/dashboard/configuracoes/plano`** (a tela nasceu em `86-31` sem ela, porque `tenant_invoices` não existia). Cliente vê as próprias faturas; RLS `org_id = user_org_id() AND is_admin()`.
- Exportação CSV; geração do ciclo em lote.
**Dep:** 86-42a, 86-41 · **Est:** G · **Executor:** @dev

#### 86-44 — `/platform` completo
...
- Todas as rotas passam por `withPlatformAdmin` e `platformQuery` (`86-42a`).
...
**Dep:** 86-43 · **Est:** G · **Executor:** @dev + @ux-design-expert

#### 86-45 — ~~Impersonation auditada~~ — **REMOVIDA por decisão de produto (D14, 2026-07-30)**

**O ID 86-45 fica permanentemente vago e NÃO deve ser reaproveitado.** Isto não é lacuna, esquecimento nem story esquecida no meio da numeração — é registro deliberado de uma remoção, para que ninguém reabra o assunto achando que caiu por descuido.
...
**Por quê:** o dono do produto decidiu que a Trifold não terá capacidade de "ver como o cliente". Suporte opera apenas com metadados e agregados via `PLATFORM_READABLE_TABLES` (`86-42a` entrega o mecanismo; `86-42b` fecha a lista).
...
**Nenhuma outra story dependia de 86-45.** Verificado: a única menção era em 86-14, sobre reaproveitamento do wrapper de escopo, e essa story permanece obrigatória por razão própria (o piso das 166 rotas service-role).

#### 86-46 — Cron `subscription-lifecycle` (nunca suspende)
...
- **Depende de 86-13** (convenção de path `{org_id}/…`): com os objetos sob o prefixo da org, a exclusão é enumerável e verificável por prefixo. Sem 86-13, não há como provar que sobrou zero objeto — por isso a dependência é dura, não conveniência.
...
**Dep:** 86-43, **86-13** · **Est:** M · **Executor:** @dev + @data-engineer
...
#### 86-47 — `org_integrations` + Vault + `resolveIntegration` com fallback explícito
...
**Dep:** 86-44 · **Est:** M · **Executor:** @dev + @data-engineer

#### 86-48 — Roteamento reverso de webhook por tenant (WhatsApp e Meta), com dual-run
...
- **Dual-run** antes do cutover: resolve pelos dois caminhos, usa o legado, loga divergência (mesmo padrão de 86-20).
...
**Dep:** 86-47, 86-24 · **Est:** G · **Executor:** @dev

#### 86-49 — Sienge e Resend por org + `/platform/orgs/[id]/integracoes`
...
- (O item "convite de usuário só cria usuário na org de quem convida" **saiu desta story e foi para 86-22**, Onda 2 — ver justificativa lá.)
**Dep:** 86-47 · **Est:** M · **Executor:** @dev
...
#### 86-50 — Provider de gateway contra a interface existente
...
PRE-0 (PR #308 em produção)
  └─ 86-1 ─ 86-2 ──┬─ [86-4 86-5 86-6 86-7] ─────────────────┐   (Lote 1 — paralelo)
                   ├─ 86-8 ─ 86-9 ─ 86-10 ───────────────────┤   (policies por domínio)
                   ├─ 86-11 ─ 86-12 ─ 86-13 ─────────────────┤   (Lote 2 — Storage)
                   ├─ 86-14 ─ 86-15 ────────────────────────┤   (Lote 3 — piso de código)
                   ├─ 86-16 ────────────────────────────────┤   (platform_admin / P4)
PRE-1 ─ 86-3 ──────┴─ 86-17 ──────────────────────────────────┴─ 86-18  ← gate bloqueante, baseline=0
                                                                   │
  86-19 ─ 86-20 ─ 86-21 ─ 86-22 ─ 86-23 ─ 86-24 ─ 86-25 ───────────┤   ONDA 2
                                                                   │
  86-26 ─ 86-27a ─┬─ 86-27b🔒 (preço) ─┐                          │
                  └─ 86-28 ─ 86-29 ─ 86-30 ─ 86-31 ─┴─ 86-32 ─────┤   ONDA 3 (🟢 vendável só com 27b)
                                                                   │
  86-33 ─ 86-34 ─ 86-35 ─ 86-36 ────────────────────────────────────┤   ONDA 4 (±5% × 14d)
                                                                   │
  86-37 ─ 86-38 ─ 86-39 ─ 86-40 ─ 86-41🔒 ────────────────────────┤   ONDA 5 → 💰 COBRÁVEL
                                                                   │
  86-42a ─┬─ 86-42b🔒 (lista/PEND-4)                              │
          └─ 86-43 ─ 86-44 ─ 86-46   (86-45 removida por D14) ────┤   ONDA 6
                                                                   │
  86-47 ─ 86-48 ─ 86-49  (sob demanda)    86-50  (sob demanda) ────┘   ONDAS 7-8
```

**Caminho crítico (a corrente mais longa, sem folga):**

```
86-1 → 86-2 ─┬─ 86-8 → 86-9 → 86-10 ──┐
   (PRE-0     ├─ 86-11 → 86-12 → 86-13 ┤   ← Storage: corrente longa, e 86-46 depende de 86-13
    trava      ├─ 86-14 → 86-15 ────────┤
    o 86-2)    └─ 86-17 (PRE-1) ────────┴→ 86-18   ← gate bloqueante, baseline = 0
   → 86-19 → 86-20 → 86-21 → 86-22 → 86-24 → 86-25
   → 86-26 → 86-27a → 86-28 → 86-31 → 86-32        ← fim da Onda 3
   → 86-33 → 86-34 → 86-36
   → 86-37 → 86-38 → 86-39 → 86-40                 ← enforcement de cota

Marcos comerciais, que NÃO estão no caminho crítico técnico:
  🟢 vendável  = fim da Onda 3 **+ 86-27b** (preço)  → travado por PEND-1 e PEND-1c
  💰 cobrável  = fim da Onda 5 **+ 86-41** (excedente) → travado por PEND-1b
```

As três correntes paralelas da Onda 1 (policies, Storage, piso de código) foram omitidas do caminho crítico anterior, assim como 86-27a. A corrente de Storage é a mais longa das três e é a que dita, na prática, quando 86-18 fecha.
...
**Correção importante sobre o Storage.** Uma versão anterior deste epic dizia que "o Storage não bloqueia 86-18", o que contradizia o próprio `Dep:` de 86-18 (**"86-4 … 86-17, todas"**). **Vale o `Dep:`: 86-11/12/13 estão dentro da Onda 1 e bloqueiam 86-18.** A frase anterior estava errada e era perigosa por uma razão concreta: **86-46 (exclusão pós-cancelamento, D13) depende duramente de 86-13** — sem a convenção de path `{org_id}/…`, a exclusão de Storage não é enumerável nem verificável por prefixo. Desescopar o Storage da Onda 1 com base naquela frase derrubaria silenciosamente a fundação da exclusão LGPD, quatro ondas depois e sem ninguém perceber.
...
| PRE-5 | **PEND-4 respondida** | 86-42b | nenhuma story existente — mas a resposta pode **criar** uma story G de acesso pontual (§2.2). Precisa chegar **antes de a Onda 6 ser planejada**, não antes de começar | aberta |
...
- [ ] `lib/billing/**` e as tabelas do Epic 78 permanecem intocadas, exceto a re-ancoragem de policy em 86-16.
...
- [ ] `platform_audit_log` nasceu append-only **na mesma migration que a criou** (86-16), nunca existiu mutável.
...
Juntas, PEND-1 e PEND-1c travam **86-27b** e, com ela, o marco **"vendável"**. Não travam mais o resto da Onda 3: a quebra de 86-27 em **27a** (catálogo e composição, livre) e **27b** (preços e cotas, 🔒) tirou 18 stories do fecho transitivo desses dois números.

### Bloqueia a Onda 5 — e trava o marco "cobrável"

**PEND-1b — Preço do atendimento excedente.**
Preço por atendimento acima da cota, e **se há faixas decrescentes** (ex.: os primeiros N excedentes a X, os seguintes a Y). Sem isso `ai_overage_tiers` fica vazio, o excedente **não é faturável** (story 86-41) e a cobrança correta não fecha.

**Também de sua alçada, em 86-37: a regra de contagem de atendimento.**
...
A suspensão é manual (D5, fechado). Falta o limiar: quantos dias em `past_due` antes de o cron alertar a Trifold? Sugestão da arquitetura: 10 dias. Baixo impacto — 86-46 pode ser draftada com o número como campo a preencher.
...
|---|---|---|
| **PEND-1** — preço dos 3 tiers | `86-27b` | 🟢 vendável |
| **PEND-1c** — cota de atendimentos/mês por tier *(nova)* | `86-27b` | 🟢 vendável |
| **PEND-1b** — preço do excedente + faixas | `86-41` | 💰 cobrável |
| **Regra de contagem de atendimento** | `86-37` (não mais `86-33` — a Onda 4 anda sem ela) | 💰 cobrável |
| **PEND-4** — o que o super-admin vê | `86-42b`, e pode **criar** uma story G nova | planejamento da Onda 6 |
| **PEND-6** — dias em `past_due` | `86-46` (campo a preencher, não bloqueia draft) | — |
| **PEND-8** — existe trial? | `86-21` (parâmetro, não bloqueia draft) | — |
| **PEND-9** — nomes `lib/tenancy`/`lib/revenue` | nada; renomeação de ~30 arquivos se mudar depois | — |
| **PRE-1** — Supabase descartável | `86-3`, `86-17` → **e o epic inteiro a partir de 86-18** | todos |
| **PRE-0** — PR #308 em produção | `86-2` (**não** `86-1`) | todos |
...
| PEND-2 | Medir custo real por baixo da venda em atendimento? | **Sim** → D11 | 2026-07-30 | 86-33, 86-34, 86-35, 86-37 |
| PEND-7 | Política no 100% da cota | **`overage`** para `active`, `hard_stop` em trial, hard cap 3× → D12 | 2026-07-30 | 86-38 |
| PEND-5 | Retenção pós-cancelamento | **90 dias de export, depois exclusão definitiva** → D13 | 2026-07-30 | 86-46 |
| PEND-3 | Impersonation | **Não haverá** → D14 | 2026-07-30 | Removeu 86-45; ver §3.2 |

---

## 15. Handoff para o Story Manager (@sm)

"Desenvolver as stories detalhadas do Epic 86 (SaaS Multi-Tenant). Considerações-chave:

- **Este epic é o 86; as stories são `86-1` … `86-50`.** Os prefixos 84 e 85 pertencem a dois fixes pontuais não relacionados (`84-1-fix-created-by-auth-id-email`, `85-1-fix-subject-ausente-email-templates-api`) — não confundir nem sobrescrever.
- **As três stories com 🔒 são `86-27b`, `86-41` e `86-42b`.** Não draftar nenhuma delas: exigiriam inventar preço (PEND-1/PEND-1b), cota (PEND-1c) ou a lista de tabelas do `/platform` (PEND-4), e o Artigo IV proíbe. **`86-27a` e `86-42a` — as metades livres — estão liberadas**, e juntas destravam quase toda a Onda 3 e a Onda 6.
...
- Reuso obrigatório (IDS: REUSE > ADAPT > CREATE): `MODULE_LABELS`/`MODULE_DESCRIPTIONS` em 86-29; `unstable_cache` de `permissions.ts` em 86-28; padrão de lembrete do Epic 78 em 86-39; padrão de RLS de `131_imobiliarias.sql` em 86-16.
