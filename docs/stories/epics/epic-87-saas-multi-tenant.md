...
updated_at: 2026-08-05  # renumeração 86→87 (colisão com epic-86-meta-capi-tracking, já mergeado)
created_by: Morgan (@pm)
priority: P0
tipo: Brownfield Enhancement (PRD-level, multi-story, 8 ondas)
numbering_note: "epic 87 — prefixos 84, 85 e 86 pertencem a trabalho não relacionado; renumerado de 86 em 2026-08-05 (ver §0.1)"
objetivo_negocio:
...
depends_on:
  - "PRE-0 (externo): PR #308 / migration 209_hotfix_rls_org_scope.sql aplicada em PRODUÇÃO. Precede 87-2 em diante — NÃO bloqueia 87-1 (criar a esteira de CI independe de migration aplicada)."
  - "PRE-1 (externo): projeto Supabase descartável criado (autorizado pelo dono do produto, ainda não existe). Bloqueia 87-3 e 87-17."
  - "PRE-2 (fato): não existe CI nem husky no repo (.github/ só tem agents/). A esteira é criação do zero, não configuração."
  - "PRE-3 (comercial): preço dos 3 tiers (PEND-1), cota de atendimentos por tier (PEND-1c) e preço do excedente (PEND-1b). Bloqueiam 87-27b e 87-41 — e SÓ elas, após a quebra 27a/27b."
fontes:
...
stories_planned: [87-1 .. 87-50, exceto 87-45 (removida por D14); 87-27 e 87-42 quebradas em a/b — 51 stories ativas]
open_questions: [PEND-1, PEND-1b, PEND-1c, PEND-4, PEND-6, PEND-8, PEND-9, regra-de-contagem-de-atendimento]
resolved_questions: [PEND-0 (epic 87), PEND-2 → D11, PEND-3 → D14 (não), PEND-5 → D13, PEND-7 → D12]
---

# Epic 87 — Trifold CRM → SaaS Multi-Tenant

## 0. Antes de qualquer coisa: dois avisos operacionais

### 0.1 Numeração — renumerado de 86 para 87 em 2026-08-05

**Este epic é o 87.** Ele nasceu como 84, foi renumerado para 86 em 2026-07-30 e para **87** em 2026-08-05. A segunda renumeração tem uma causa concreta que vale registrar, porque ela vai se repetir: **este epic ficou 3 dias aberto em PR sem merge (PR #337, de 02/08 a 05/08) enquanto a `main` andou 21 commits, e o número 86 foi reivindicado nesse intervalo** por um epic que já foi mergeado e está em produção.

Os prefixos 84, 85 e 86 pertencem a trabalho que **não faz parte deste epic** e continua válido onde está:

| Prefixo | Dono | Situação |
|---|---|---|
| `84-1-fix-created-by-auth-id-email.story.md` | fix isolado do `created_by` | na `main` |
| `epic-84-qualificacao-lead.md` + `84-1-qualificacao-lead-schema-…` / `84-2-qualificacao-lead-ui` | Qualificação Comercial do lead | PRs #362 e #366, abertos — **colidem entre si no ID `84-1`; é problema daquele epic, não deste** |
| `85-1-fix-subject-ausente-email-templates-api.story.md` | fix isolado de template de e-mail | na `main` |
| `epic-86-meta-capi-tracking.md` + `86-1` … `86-8` | Conversions API / rastreamento Meta | **mergeado**, `86-1` já implementado (commit `97de766c`, migration 215) |

As stories deste epic são `87-1` … `87-50`, das quais **51 estão ativas** (com `87-27` e `87-42` quebradas em `a`/`b`): o ID `87-45` foi **removido por decisão de produto** (D14, sem impersonation) e deliberadamente **não** foi reaproveitado — ver §3.2. `87-45` não é lacuna nem esquecimento.

> Ao ler referências cruzadas: qualquer `84-N`, `85-N` ou `86-N` citado neste documento aponta para trabalho **de fora** deste epic. **Nenhuma story deste epic usa os prefixos 84, 85 ou 86.**

> ⚠️ **Lição operacional, e ela é do processo, não deste documento.** A convenção "próximo número livre = maior existente + 1" resolve a colisão no instante em que o epic é escrito e **não a impede depois**: entre draftar e mergear, qualquer story pontual pode tomar o número. Só há duas saídas — mergear o epic rápido (mesmo em `Draft`, para o número ficar reservado no repo), ou reservar a faixa numérica em algum lugar que a `main` enxergue. Este epic pagou o preço duas vezes; renumerar 1037 referências cruzadas é barato com `perl -pi`, mas revisar o que o sed não alcança não é.
...
| **`lib/billing/**` e `platform_services`/`service_billing_*` (Epic 78)** | Domínio diferente: é o custo que a **Trifold paga**. Não ganha `org_id`. Só é tocado em 87-16, para re-ancorar a policy. |
...
| D2 | Gateway de pagamento | Nenhum agora; cobrança fora do sistema, atrás de `BillingProvider` | `ManualBillingProvider` em 87-43; Onda 8 sob demanda |
| D3 | Onboarding | Provisionamento pela Trifold via `/platform`; sem signup público | 87-21, 87-22 |
| D4 | Usuário × org | 1 usuário = 1 empresa; Trifold acessa via `platform_admin` | Fecha Q9/P10; 87-16 |
| D5 | Suspensão por inadimplência | **Manual**, nunca automática | 87-46: cron alerta, não suspende |
| D6 | Ambiente de teste | Supabase descartável **autorizado** (ainda não existe) | PRE-1; 87-3 |
| D7 | Planos | **3 tiers acumulativos**, exatamente a composição sugerida em §11.3 Q8 | 87-27a (composição) + 87-27b (preço) — ver §3.1 |
...
| # | Tema | Decisão | Consequência no epic |
|---|---|---|---|
| D11 | Medição de IA por baixo da venda | **Sim** — vender em atendimento **e** persistir custo real por chamada, internamente | Desbloqueia 87-33, 87-34, 87-35, 87-37; ver §4 |
| D12 | Política no 100% da cota | **`overage`** para `active` (a Nicole nunca para; excedente vai para a fatura); `hard_stop` só em trial; **hard cap de 3×** a cota como proteção contra loop de bug | Desbloqueia 87-38; fecha Q2/Q2 do ADR-007 §4 |
| D13 | Retenção pós-cancelamento | **90 dias de janela de export, depois exclusão definitiva** | Desbloqueia 87-46; exige exclusão real **incluindo Storage** e cláusula de contrato |
| D14 | Impersonation | **NÃO haverá impersonation.** Suporte opera só com metadados e agregados | **Remove a story 87-45**; invalida parcialmente o ADR-004; eleva `87-42b` e PEND-4 (ver §3.2) |
...
- A story `87-45` inteira: `createImpersonationClient`, o overlay em `getServerUser()`, o banner vermelho com cronômetro, `platform_impersonation_sessions`, o e-mail de notificação ao admin do cliente, os 7 controles do ADR-004.
...
4. **`createOrgScopedAdminClient` (87-14) continua obrigatório.** Ele foi projetado como o wrapper reaproveitado pela impersonation, mas sua razão principal sempre foi outra: o piso de isolamento das 166 rotas em service-role (R1). D14 não o afeta.
...
|---|---|
| Camada `platform_admin` (tabela `platform_admins`, níveis `owner`/`operator`/`support`, `requirePlatformAdmin`, auditoria append-only, `withPlatformAdmin` como decorador) | **válida** — implementada em `87-16` e `87-42a` |
...
Custo de implementar: **zero incremental** — e foi o que fechou a decisão. O proxy sobre `createAnthropicClient()` (Onda 4, story 87-34) já lê `response.usage` para contar o atendimento; gravar `cost_micro_usd` na mesma linha é a mesma escrita. As tabelas já preveem as duas colunas (`saas-multi-tenant.md` §2.5).

**Consequência de aceite, não opcional:** `/platform/usage` (87-35) tem de mostrar **margem por org**, não só consumo. Sem essa tela, o custo é coletado e ninguém olha — e a decisão perde a razão de existir.

### 4.2 Como fica modelado

| Dimensão | Unidade | Onde vive | Quem vê |
|---|---|---|---|
| Cota vendida | atendimentos/mês | `plans.ai_quota_atendimentos` e `org_billing_periods.consumed_atendimentos` — **nomes próprios, não `*_credits` reinterpretado** (ver 87-33: no ADR-007 "crédito" significa custo real, e reaproveitar o identificador criaria ambiguidade em cima de uma linha de fatura) | Cliente (`/dashboard/configuracoes/plano`) e Trifold |
...
| FR-28 | `/platform` só consulta a lista fechada `PLATFORM_READABLE_TABLES`, validada em runtime por `platformQuery()` (nasce em `87-22`, endurecida em `87-42a`, lista fechada em `87-42b`) e por teste que varre `app/api/platform/**` | §3.4 |
...
| NFR-5 | **RLS é a rede, não o piso.** O piso é o filtro explícito de org + lint. Ambos obrigatórios — 166 de 285 handlers bypassam RLS. | P7; §8.4 |
...
|---|---|---|---|---|
| **R1** | **Vazamento cross-tenant por rota service-role.** 166 dos 285 handlers usam `createAdminClient()` e ignoram RLS. **O gate de CI não cobre essa superfície** — o banco fica correto e a aplicação continua podendo vazar. | **alta** | **crítico** | Stories 87-14 e 87-15: `createOrgScopedAdminClient` + ESLint rule, migração priorizada pelas rotas que tocam PII. É o risco nº 1 do projeto e a razão de o gate ter par de lint, não só SQL. |
| **R2** | **Ausência de CI e de staging.** Nada verifica nada automaticamente hoje; o Supabase de dev aponta para prod. Sem staging, os testes cross-tenant (que criam e apagam orgs) não têm onde rodar. | **alta** | alto | Onda 0 cria a esteira do zero (87-1, 87-2). PRE-1 (Supabase descartável) desbloqueia 87-3/87-17. Enquanto não existir, o job de isolamento fica `continue-on-error` — proteção bem mais fraca, registrada como risco aceito. |
| **R3** | **Dois arquivos concentram o risco de regressão.** `lib/permissions.ts` (15 linhas que decidem se **todo mundo** vê **qualquer coisa**) e `packages/ai/src/client/anthropic.ts` (se o proxy lançar, a Nicole para). `lib/auth.ts` / `getServerUser()` **saiu desta lista por D14**: sem overlay de impersonation, este epic não toca a função por onde passam 100% das requests autenticadas — é o único benefício técnico de não ter impersonation. | média | **crítico** | QA gate reforçado nas stories 87-28 e 87-34. Teste unitário obrigatório do **caminho de falha**, não só do caminho felizardo: `fail-open` em entitlement, `try/catch` total no proxy de IA. |

### 8.2 Demais riscos

| # | Risco | Prob. | Impacto | Mitigação |
|---|---|---|---|---|
| R4 | Policy nova mais restritiva quebra tela da Trifold em produção | média | alto | Lotes pequenos por domínio (87-8/9/10), QA gate por lote, migration inversa documentada (NFR-8), Trifold como detector rápido |
...
| R6 | Medição de IA quebra a Nicole em produção | média | **crítico** | `ctx` opcional, proxy com `try/catch`, shadow mode, reconciliação de ±5% (87-36) |
| R7 | **Margem negativa invisível** por vender atendimento e não medir custo | **média** | **médio-alto** | **Mitigado por D11:** custo real por chamada em `ai_usage_events` + margem por org em `/platform/usage` (87-35). O risco residual passa a ser não *olhar* a tela — por isso a margem é AC de 87-35, não item de backlog |
...
| R12 | Deriva entre `getHardcodedPermissions()` (TS) e o seed de provisionamento (SQL) | média | médio | `role_default_permissions` como fonte única + teste de paridade (87-21) |
| R13 | Storage: mudar bucket para privado quebra URLs já distribuídas a clientes | **alta** | alto | 87-12 exige varredura das referências gravadas no banco antes do flip; URL assinada com fallback e período de sobreposição |
...
| R15 | Colisão silenciosa de `CREATE OR REPLACE FUNCTION` entre migrations | média | alto | Asserção **R10** do gate (87-2). Quase reverteu a Story 75-226 neste ciclo |

---

## 9. O gate de CI — asserções consolidadas

Story 87-2 implementa `scripts/gate-tenancy.ts` (`pnpm gate:tenancy`). Fonte da verdade do schema: introspecção via Supabase Management API (`SUPABASE_MANAGEMENT_PAT`, padrão já usado no projeto), com fallback para snapshot versionado. **Não parsear os 222 arquivos de migration** — o schema efetivo é o que importa.
...
**Resolução adotada:** `87-2` cria um gerador novo, `scripts/generate-schema-snapshot.ts`. Não é correção do script existente — são coisas diferentes.

**O que NÃO muda, para evitar correção excessiva:** `scripts/sync-schema.sh` **é** corretamente reaproveitável em `87-3`, para aplicar as 222 migrations num projeto Supabase novo. A imprecisão da arquitetura é só sobre a alegação de snapshot; o script serve bem ao propósito para o qual foi escrito.
...
| **P5** `privacy_consents` sem `org_id` | 🟡 **só a partir de `87-4`** | R1-R3 avaliam apenas tabelas que **já têm** `org_id`. Enquanto a coluna não existir, o gate é cego para esta tabela **por construção** — não por falha. Depois que `87-4` a adicionar, ela entra no alcance de R1 e R2 e passa a ser protegida contra regressão |
| **P4** tabelas de custo do Epic 78 | ❌ | são tabelas de **plataforma**, legitimamente sem `org_id` — vão para a allowlist. O controle é `87-16`, não o gate |
| **P7** Storage sem escopo de org | ❌ | o gate lê `pg_policies` de `public`; policies de `storage.objects` e buckets públicos estão fora do alcance. Controle: `87-11`/`87-12`/`87-13` |
| **P9** UNIQUEs globais que colidem | ❌ | é forma de constraint, não isolamento. Controle: `87-5` |
| **P10** `users.auth_id UNIQUE` | ❌ | decisão de produto (D4), não defeito |
| **P11** 16 índices faltando | ❌ | performance, não isolamento. Controle: `87-6` |
| **P12** `whatsapp_pricing` global | ❌ | dado global aceito; decisão registrada em `87-16` |
...
**Ressalva obrigatória, escrita no próprio relatório do gate:** este gate valida o **banco**, não o código. Não vê query em service-role sem `.eq("org_id")` — a maior superfície de risco (R1/CON-4). O par indispensável é a regra de ESLint da story 87-14.
...
|---|---|
| 87-2 | **ADOTADO pelo @sm em 2026-08-03** → `87-2a` (motor + R1-R4 + snapshot + `known-tables`) · `87-2b` (R5-R9: grants, `relkind`, colisão) · `87-2c` (baseline + allowlist + wiring de CI) |
| 87-12 | preparação reversível (varredura de referências + leitor por URL assinada) / **flip dos buckets** (irreversível na prática, R13 alto) |
| 87-13 | expand (convenção + escritas novas) / migrate (mover objetos + reescrever referências) / contract (policy por path) |
| 87-20 | **dual-run de 7 dias** / cutover + contract |
| 87-44 | são 6 telas distintas — é um mini-épico, não uma story |
| 87-48 | dual-run de roteamento / cutover |
...
**Princípio aplicado em todos os casos abaixo, e vale citar:** *o artefato nasce onde é primeiro usado, e é refinado depois.* Foi assim que `platform_audit_log` foi de `87-42a` para `87-16`, `org_billing_periods` de `87-33` para `87-26`, e `platformQuery()` de `87-42a` para `87-22`. Atributo de segurança (append-only, `NOT NULL`, escopo de org) é **atributo de nascimento**, nunca refino posterior.

> **Nota de leitura:** os IDs `87-2a`/`87-2b`/`87-2c` referem-se às sub-stories já draftadas pelo @sm em `docs/stories/` (a quebra sugerida no §10 foi adotada em 2026-08-03). Elas **não** têm cabeçalho próprio neste epic, que mantém `87-2` como unidade de planejamento — não são referências órfãs.

| Artefato | Criado em | **Estendido por** | 1ª menção | Nota |
|---|---|---|---|---|
| `.github/workflows/ci.yml` | `87-1` | `87-2c` (job `tenancy-gate`), `87-17` (job `isolation`), `87-18` (torna o gate bloqueante) | `87-1` | Incremental: cada onda pendura um job. Nunca reescrever o workflow — **acrescentar job** |
| `scripts/generate-schema-snapshot.ts` | `87-2a` | `87-17` (os testes cross-tenant são data-driven pelo snapshot) | `87-2a` | Criado do zero — **não** existe hoje, ver §9.1 |
| `scripts/gate-tenancy.ts` | `87-2a` (motor + R1-R4) | `87-2b` (R5-R9), `87-2c` (baseline, allowlist, saída de PR), `87-26` (liga R10), `87-34` (liga R11), `87-42a` (liga R12) | `87-2a` | **O artefato mais estendido do epic depois da `PLATFORM_READABLE_TABLES`.** Três regras são ligadas por ondas futuras |
| `docs/audits/tenancy-known-tables.json` | `87-2a` | **nenhuma — o arquivo NUNCA cresce** | `87-2a` | Ver o invariante abaixo. É a linha mais importante desta tabela |
| `docs/audits/tenancy-allowlist.yml` | `87-2c` | `87-10` (16 tabelas service-role-only), `87-26` (tabelas de plataforma sem `org_id`), `87-18` (congela o conteúdo na saída da Onda 1) | `87-2` | Cresce **só** com `reason:` preenchido e diff revisável |
| `rls-gate-baseline.json` | `87-2c` | `87-8`, `87-9`, `87-10` (cada lote abaixa a contagem), `87-18` (zera e torna bloqueante) | `87-2a` | Incremental por desenho — é a catraca |
| `createOrgScopedAdminClient()` | `87-14` | `87-15` (migra as rotas; ESLint `warn` → `error`) | `87-14` | |
| `platform_admins` | `87-16` | — | `87-16` | Atômico |
| `platform_audit_log` + `platform_audit()` | `87-16` | — | `87-16` | **Movido de `87-42a` para cá (B1).** Nasce append-only na mesma migration: imutabilidade é atributo de nascimento |
| `kanban_stages.semantic_key`, `properties.semantic_key` | `87-19` | `87-20` (resolver, dual-run, cutover, contract) | `87-19` | expand aqui, migrate/contract lá |
| `role_default_permissions` | `87-21` | — | `87-21` | Atômico |
| `org_integrations` | `87-21` (esqueleto) | `87-47` (`secret_ref`/Vault, índices de roteamento reverso, `resolveIntegration`) | `87-21` | **Movido de `87-47` (B2)** — a Onda 7 é sob demanda e pode nunca acontecer |
| `provision_org()` | `87-21` (`p_plan_id` NULL) | `87-31` (`p_plan_id` não-nulo + cria a assinatura) | `87-21` | Fatiado para quebrar o ciclo O2↔O3 (B3) |
| `PLATFORM_READABLE_TABLES` | `87-22` (provisória mínima) | `87-31`, `87-35`, `87-44` (cada tela acrescenta as suas tabelas) · `87-42a` **consolida e audita** · `87-42b` **fecha** 🔒 | `87-22` | **O artefato mais fatiado do epic — 6 stories.** É o que o E1 revelou |
| `platformQuery()` | `87-22` | `87-42a` (endurece + regra R12 do gate) | `87-22` | **Movido de `87-42a` para cá (F3)** — `87-35` o exige na Onda 4 |
| `sellable_modules`, `plans`, `plan_modules`, `plan_limits`, `org_module_grants`, `org_limit_overrides` | `87-26` | `87-27a` (seed do catálogo e composição) · `87-27b` (preços e cotas) 🔒 | `87-26` | DDL atômico; o seed é que é fatiado |
| `org_subscriptions` | `87-26` | `87-31` (cria as linhas), `87-46` (lifecycle de status) | `87-21` (menção **negativa**) | |
| `org_billing_periods` | `87-26` | `87-33` (`cost_micro_usd`), `87-37` (`consumed_atendimentos`), `87-41` (`overage_*`) | `87-21` (menção **negativa**) | **Movido de `87-33` para cá (F1).** Três ondas escrevem colunas nele |
| `ai_usage_events`, `ai_usage_daily`, `record_ai_usage()` | `87-33` | `87-37` (incremento de atendimento dentro da RPC) | `87-33` | |
| `/dashboard/configuracoes/plano` | `87-31` | `87-39` (barra de cota), `87-43` (seção de faturas) | `87-31` | |
| `withPlatformAdmin` | `87-42a` | `87-44` (aplica em todas as rotas de `/platform`) | `87-42a` | |
| `tenant_invoices`, `tenant_invoice_lines` | `87-43` | `87-50` (campos `provider_*` do gateway) | `87-31` (menção **negativa**) | |
...
>
> AC obrigatória em `87-2a`: comentário no topo do arquivo com esse invariante, e o **próprio gate falha** se `known-tables.json` tiver mais entradas que na revisão anterior.

**As três "menções negativas" são deliberadas e devem permanecer.** São ACs que dizem *"esta story NÃO cria/exibe X — quem cria é Y"*: `87-21` sobre `org_billing_periods`/`org_subscriptions`, `87-31` sobre `tenant_invoices`, `87-33` sobre `consumed_atendimentos`. Elas existem para impedir que o @sm ou o @dev assumam que o artefato está disponível. Uma menção negativa vale mais que a ausência de menção.
...
#### 87-2 — Gate de tenancy (R1-R12) com baseline e catraca, não-bloqueante
...
**Dep:** 87-1 · **Est:** G · **Executor:** @dev + @data-engineer

#### 87-3 — Projeto Supabase descartável + harness de isolamento
...
#### 87-4 — Lote 1 / P5: `privacy_consents` com escopo de org
...
**Dep:** 87-2 · **Est:** P · **Executor:** @data-engineer

#### 87-5 — Lote 1 / P9: UNIQUEs compostos com `org_id`
...
**Dep:** 87-2 · **Est:** M · **Executor:** @data-engineer

#### 87-6 — Lote 1 / P11: 16 índices `CONCURRENTLY` iniciando em `org_id`
...
#### 87-7 — P13 + varredura final de views: `search_path` e `security_invoker`
...
#### 87-8 — Policies org-scoped, lote A: leads, conversas, mensagens (PII primeiro)
...
**Dep:** 87-2 · **Est:** G · **Executor:** @data-engineer

#### 87-9 — Policies org-scoped, lote B: agenda, obras, portal do cliente, pastas
**Objetivo:** mesmo padrão do lote A no domínio de obras/agenda/portal.
**AC:**
- **O conjunto de tabelas deste lote é a partição B de `docs/audits/rls-gate-baseline.json`** (agenda, obras, portal, pastas) — enumerada no baseline, não neste epic: a lista só é conhecível depois que 87-2 emitir o arquivo.
...
> **Nota de execução:** draftável agora; **não promover a `Ready`** antes de 87-2 emitir o baseline.
**Dep:** 87-8 (padrão estabelecido) · **Est:** G · **Executor:** @data-engineer

#### 87-10 — Policies org-scoped, lote C: financeiro, imob, campanhas, marketing, `system_events`
...
- Migration inversa documentada.
> **Nota de execução:** draftável agora; **não promover a `Ready`** antes de 87-2 emitir o baseline.
**Dep:** 87-8 · **Est:** G · **Executor:** @data-engineer

#### 87-11 — Lote 2a / P7a: policies de Storage ancoradas em org
...
#### 87-12 — Lote 2b / P7b: buckets com PII → privados + URL assinada
...
**Dep:** 87-11 · **Est:** G · **Executor:** @dev + @data-engineer

#### 87-13 — Lote 2c / P7c: convenção de path `{org_id}/…` + migração de objetos
...
**Dep:** 87-12 · **Est:** G · **Executor:** @dev + @data-engineer

#### 87-14 — Lote 3 / P8: `createOrgScopedAdminClient()` + regra de ESLint
**Objetivo:** criar o **piso** de isolamento. RLS é a rede (NFR-5): 166 de 285 handlers a bypassam.
**AC:**
...
- Regra entra como **warn** nesta story; vira **error** ao fim de 87-15.
...
**Dep:** 87-2 · **Est:** M · **Executor:** @dev

#### 87-15 — Migração das rotas service-role que tocam PII para o client escopado
...
**Dep:** 87-14 · **Est:** G · **Executor:** @dev

#### 87-16 — Lote 3 / P4: papel `platform_admin`, trilha imutável + re-ancoragem das tabelas do Epic 78
...
- **Por que a tabela nasce aqui e não em `87-42a`, onde o mecanismo é construído:** ela passa a ser **escrita** já em 87-21 (`action='org.create'`, Onda 2) e 87-31 (Onda 3). Criá-la sem o `REVOKE` e endurecê-la quatro ondas depois deixaria a trilha do provisionamento do **primeiro cliente pagante** forjável e apagável por `service_role` — violando o NFR-13 deste epic por 4 ondas. Imutabilidade é atributo de nascimento, não de refino.
...
- **Esta story precede a criação da primeira conta admin de cliente (87-22/87-25).** Gatilho do dano é a Onda 2, não a Onda 6.
**Dep:** 87-2 · **Est:** M · **Executor:** @dev + @data-engineer

#### 87-17 — Testes de isolamento cross-tenant, data-driven
...
- Roda no CI contra o projeto descartável (87-3), não contra produção.
- Job passa a `continue-on-error: false` ao fim da story.
**Dep:** 87-3, 87-8, 87-9, 87-10 · **Est:** M · **Executor:** @qa + @dev

#### 87-18 — Baseline a zero + gate bloqueante
...
**Dep:** 87-4 … 87-17 (todas) · **Est:** P · **Executor:** @qa + @devops
...
#### 87-19 — Expand: chaves semânticas de stage e imóvel (aditivo, backfill)
...
**Dep:** 87-18 · **Est:** M · **Executor:** @data-engineer

#### 87-20 — Resolver de stage/imóvel + dual-run + cutover + contract (14 arquivos)
...
**Dep:** 87-19 · **Est:** G · **Executor:** @dev

#### 87-21 — `role_default_permissions` + `provision_org()` idempotente
...
- **`p_plan_id` é NULLABLE e na Onda 2 é sempre NULL.** A arquitetura é explícita — §10, Onda 2: *"Mínimo do `/platform`: layout + guard + `/platform/orgs` + `/platform/orgs/new`. **Sem plano, sem fatura.**"* — e a assinatura herdada da §7.4 contradizia isso. **A incoerência é da arquitetura; a resolução é esta:** a Onda 2 provisiona org **sem plano**, sem `org_subscriptions` e sem `org_billing_periods`. Esses passos entram quando `plans` existir (87-26/27a, Onda 3), como AC de 87-31.
- Consequência de ordem, e é a razão de ser desta escolha: sem ela o grafo de artefatos fecha um **ciclo** (87-25 → 87-27 → 87-26 → 87-25) e o critério de saída da Onda 2 passaria a depender de um preço em aberto — a inversão exata que o §1.1 proíbe.
...
- **Esqueleto de `org_integrations` criado nesta story** (tabela + `org_id` + `provider` + `status` + `config jsonb` com identificadores públicos + `UNIQUE (org_id, provider)` + RLS): `provision_org` a semeia, e ela **não pode nascer na Onda 7**, que é declarada sob demanda e pode nunca acontecer. O que fica para 87-47 é o que é de fato da Onda 7: `secret_ref`/Vault, índices UNIQUE de roteamento reverso, `resolveIntegration`, `platform_shared`. É o mesmo expand→migrate que o epic exige de todo o resto (NFR-1).
...
**Dep:** 87-19, 87-16 · **Est:** G · **Executor:** @data-engineer + @dev

#### 87-22 — `/platform` mínimo: layout, guard, lista de orgs, wizard de provisionamento
...
- **O wizard não tem seleção de plano nem de módulos extras nesta onda** — `plans`, `plan_modules` e `org_module_grants` só existem a partir de 87-26 (Onda 3). Os campos comerciais entram em 87-31, que é dona da tela de plano. Antecipá-los aqui criaria dependência de artefato de uma onda futura, que é o defeito que esta correção fecha.
...
- **`platformQuery(table, orgId)` e a constante `PLATFORM_READABLE_TABLES` nascem nesta story**, junto da primeira tela de `/platform`, com a lista provisória mínima (`organizations`, `users`) e o comentário de topo *"lista PROVISÓRIA — consolidada por 87-42a, fechada por 87-42b"*.
- **Regra de crescimento da lista, válida do `87-22` em diante:** *toda story que adiciona tela de `/platform` acrescenta, no mesmo PR, as tabelas que aquela tela lê* — com uma linha de justificativa por tabela. Sem essa regra, a próxima tela bate em `platformQuery()`, é rejeitada, e o @dev contorna o mecanismo exatamente onde ele deveria valer. Quem estende: `87-31`, `87-35`, `87-44`. Quem consolida e audita: `87-42a`. Quem fecha: `87-42b`. Motivo: `87-35` (Onda 4) já exige "só via `platformQuery()`", e `87-42a` é da Onda 6 — declarar a dependência para frente inverteria a ordem de ondas. Vale o mesmo princípio do `platform_audit_log`: **o artefato nasce onde é primeiro usado**, e é refinado depois (`87-42a` adiciona o decorador, os níveis e a regra R12; `87-42b` fecha a lista). Como efeito colateral, `/platform/orgs` deixa de ser a única tela de plataforma sem guarda de leitura.
- **Convite de usuário só pode criar usuário na org de quem convida** (§8.5 da arquitetura). Movido para cá vindo de 87-49 (Onda 7, sob demanda): é vetor de escalada cross-tenant e **a primeira conta admin de cliente nasce nesta story**. Atenuante verificado no código — `packages/web/src/app/api/brokers/route.ts` já deriva `org_id` de `appUser.org_id` em todos os pontos de escrita — então o trabalho é **verificar e replicar**, não corrigir.
**Dep:** 87-16, 87-21 · **Est:** M · **Executor:** @dev

#### 87-23 — 37 crons iteram orgs (`forEachActiveOrg`) — fim do `DEFAULT_ORG_ID`
**Objetivo:** o backend deixa de assumir uma org. É onde o vazamento aconteceria (§4.6).
**AC:**
- `forEachActiveOrg(fn)` em `lib/tenancy/guard.ts`, com **isolamento de erro por org** (uma org que falha não aborta as outras) e log por org.
- 37 crons migrados; `DEFAULT_ORG_ID` removido de `cron/daily-report` e de qualquer outro ponto.
...
- Ainda **sem** entitlement (só iteração) — o filtro por módulo entra em 87-30.
**Dep:** 87-20 · **Est:** G · **Executor:** @dev

#### 87-24 — Roteamento de webhook por identificador, com dual-run
...
**Dep:** 87-23 · **Est:** M · **Executor:** @dev

#### 87-25 — Org "Trifold Sandbox" + teste de aceitação de multi-tenancy
...
- Org "Trifold Sandbox" criada em produção via `/platform/orgs/new`, **sem plano** (a Onda 2 não tem `plans` — ver 87-21/87-22).
...
- **A AC (b) da §7.4 — "vê Pipeline/Leads/Imóveis e não vê Obras/Lançamentos" — NÃO é verificável nesta onda e foi movida para 87-32.** Motivo, e é factual: `provision_org` semeia `role_permissions` para os **26 `ALL_MODULES`** em cada role; sem a camada de entitlement (87-28) o admin de uma org nova **vê tudo**, inclusive Obras e Lançamentos. Exigir (b) aqui seria uma AC insatisfazível por construção, e ela já está coberta em 87-32 ("Trifold Sandbox com plano `crm` vê exatamente os módulos do tier 10").
- Roteiro do teste versionado e re-executável a cada onda seguinte, ganhando (b) quando a Onda 3 fechar.
**Dep:** 87-22, 87-24 · **Est:** P · **Executor:** @qa
...
#### 87-26 — Modelo de dados de tenancy + entitlement derivado
...
- **`CREATE TABLE org_billing_periods`** (1 linha por org por ciclo: `period_start`/`period_end`, `quota_*`, `alert_80_at`, `alert_100_at`, `closed_at`) — **a tabela nasce aqui, não na Onda 4.** Ela é consumida por `org_entitlement_snapshot()` nesta mesma story ("linha do ciclo") e por `87-31` (Onda 3), duas ondas antes de `87-33`. Mesma classe do defeito do `platform_audit_log`: criar tarde deixa duas stories sem onde ler.
...
**Dep:** 87-25 · **Est:** M · **Executor:** @data-engineer

#### 87-27a — Seed do catálogo de módulos e da composição dos 3 tiers (**sem preço**)
...
- Colunas de preço e cota deixadas **NULL**, com `NOT NULL` adiado para 87-27b — plano sem preço não pode ser vendido, mas pode ser semeado e testado.
**Dep:** 87-26 · **Est:** P · **Executor:** @data-engineer
**Por que existe separada:** enquanto catálogo e preço eram uma story só, PEND-1 travava por transitividade **20 stories** (87-27 → 87-32 → Ondas 4, 5, 6, 7, 8). Separados, um preço em aberto trava apenas a cobrança — que é o que a tabela de PRE-3 sempre dizia e agora é verdade.

#### 87-27b — 🔒 Preços e cotas dos 3 tiers
...
**Dep:** 87-27a · **Est:** P · **Executor:** @pm (valores) + @data-engineer (seed)

#### 87-28 — Camada de entitlement no código + interseção no RBAC
...
**Dep:** 87-26 · **Est:** M em código, **alto em consequência** · **Executor:** @dev

#### 87-29 — `ModuleLockedScreen`, cadeado na sidebar e CTA de upgrade
...
**Dep:** 87-28 · **Est:** M · **Executor:** @ux-design-expert + @dev

#### 87-30 — Entitlement no lado servidor: crons e webhooks
...
**Dep:** 87-28, 87-23 · **Est:** M · **Executor:** @dev

#### 87-31 — `/platform/orgs/[id]/plano` + `/dashboard/configuracoes/plano`
...
- **Estende `PLATFORM_READABLE_TABLES`** (regra de crescimento de `87-22`) com `org_subscriptions`, `plans`, `plan_modules`, `org_module_grants` e `org_limit_overrides` — as tabelas que esta tela lê —, uma justificativa por tabela, no mesmo PR.
...
- **Sem lista de faturas nesta onda**: `tenant_invoices` só nasce em `87-43` (Onda 6). A seção de faturas é adicionada por `87-43`, na mesma story que cria a tabela. É a mesma correção da AC (b) de `87-25` — não exibir na Onda 3 o que só existe na Onda 6.
...
- Toda mutação registra a trilha via `platform_audit()` — **que já existe desde 87-16**, com `REVOKE UPDATE, DELETE` aplicado no nascimento (B1). Nenhuma escrita de auditoria acontece em tabela mutável.
**Dep:** 87-28, 87-22 · **Est:** M · **Executor:** @dev

#### 87-32 — Trifold no plano `completo-interno` + verificação de NFR-2
...
- **Herda a AC (b) de 87-25** (movida por ser insatisfazível na Onda 2): a Sandbox vê Pipeline/Leads/Imóveis e **não** vê Obras/Lançamentos.
**Dep:** **87-27a**, 87-29, 87-30, 87-31 · **Est:** **M** (o checklist das 87 páginas do dashboard não cabe em <1 dia-agente) · **Executor:** @qa
...
#### 87-33 — Modelo de dados de consumo de IA + RPC de registro
...
- **`org_billing_periods` já existe desde `87-26`** (Onda 3). Esta story **adiciona `cost_micro_usd` de forma aditiva** — mesmo padrão que `87-37` usa para `consumed_atendimentos`. Não recriar a tabela.
...
**Dep:** 87-32 · **Est:** M · **Executor:** @data-engineer

#### 87-34 — Proxy de medição na fábrica do client + `ctx` nos 12 call sites
...
**Dep:** 87-33 · **Est:** G · **Executor:** @dev

#### 87-35 — `/platform/orgs/[id]/ia` + `/platform/usage` **com margem por org**
...
- **Estende `PLATFORM_READABLE_TABLES`** com `ai_usage_daily` e `org_billing_periods` (regra de `87-22`).
...
- `/platform/usage` agrega cross-org; só via `platformQuery()` — **que já existe desde `87-22`**, com a lista ampliada aqui para incluir `ai_usage_daily` e `org_billing_periods`.
- Nenhuma PII de lead nessas telas (`PLATFORM_READABLE_TABLES`).
**Dep:** 87-34 · **Est:** M · **Executor:** @dev

#### 87-36 — Reconciliação com o oráculo do Epic 78 (gate de saída da onda)
...
#### 87-37 — Unidade de venda "atendimento": definição contável e contador do ciclo
...
- **Esta story introduz `org_billing_periods.consumed_atendimentos`** (migration aditiva) e o incremento correspondente na RPC `record_ai_usage`, que 87-33 deixou deliberadamente de fora. A UI do cliente exibe a mesma unidade do contrato, sem tradução nem estimativa.
...
**Dep:** 87-36 · **Est:** M · **Executor:** @pm (regra) + @dev

#### 87-38 — `checkAiQuota` + `pickModel(tier, verdict)` + desligamento de flows opcionais
...
**Dep:** 87-37 · **Est:** M · **Executor:** @dev

#### 87-39 — Cron `ai-quota-notify` + barra de consumo no dashboard do cliente
...
**Dep:** 87-38 · **Est:** M · **Executor:** @dev

#### 87-40 — `AI_QUOTA_ENFORCEMENT=on` com rollout escalonado
...
**Dep:** 87-39, 87-36 · **Est:** P · **Executor:** @devops

#### 87-41 — 🔒 Faturamento do excedente de atendimentos
...
- Linha `kind='ai_overage'` gerada por `buildInvoiceForPeriod` (87-43).
...
**Dep:** 87-38 · **Est:** M · **Executor:** @pm (valores) + @dev

---

### Onda 6 — Painel completo, faturamento interno e auditoria (5 stories: 87-42a, 87-42b, 87-43, 87-44, 87-46 — 87-45 removida por D14)
...
#### 87-42a — Mecanismo de plataforma: `withPlatformAdmin`, níveis e `platformQuery()`
...
- **Esta story CONSOLIDA E AUDITA a `PLATFORM_READABLE_TABLES`** — não a cria e não a entrega. A constante nasce em `87-22` e cresce incrementalmente por `87-31`, `87-35` e `87-44`, cada uma acrescentando as tabelas da própria tela (regra de crescimento de `87-22`). Aqui ela é: revisada entrada por entrada, com a justificativa de cada tabela conferida contra o que a tela realmente lê; limpa de entradas órfãs (tabela na lista que nenhuma tela consulta); e congelada contra crescimento silencioso.
- Comentário obrigatório no topo do arquivo: *"lista PROVISÓRIA — consolidada em 87-42a, fechada por 87-42b quando PEND-4 for respondida."*
- **A lista consolidada aqui ainda não é a resposta de PEND-4** — é o retrato auditado do que o produto passou a ler. `87-42b` a confronta com a decisão do dono do produto e a fecha.
- `platformQuery(table, orgId)` — **criada em `87-22`** — é endurecida aqui: rejeição em runtime contra a lista vigente, `select` explícito de colunas, nunca `select('*')` em `users`, e a regra R12 do gate passando a cobri-la.
- Teste que varre `app/api/platform/**` procurando `.from("…")` fora da lista. Regra **R12** do gate ativada.
- `platform_audit_log` e `platform_audit()` **já existem desde 87-16** — esta story consome, não cria.
- **Esta story é draftável e entregável sem a resposta do Gabriel.** O mecanismo depende de *existir uma lista*, não de saber qual é — e o bullet acima garante que existe uma. O que depende da decisão é o **conteúdo** da lista, isolado em `87-42b`.
**Dep:** 87-32 · **Est:** M · **Executor:** @dev

#### 87-42b — 🔒 A lista: o que o super-admin pode ver de um cliente
...
**Dep:** 87-42a · **Est:** P · **Executor:** @pm (decisão) + @dev (implementação)

#### 87-43 — Faturas internas + `ManualBillingProvider`
...
- **Esta story adiciona a seção de faturas a `/dashboard/configuracoes/plano`** (a tela nasceu em `87-31` sem ela, porque `tenant_invoices` não existia). Cliente vê as próprias faturas; RLS `org_id = user_org_id() AND is_admin()`.
- Exportação CSV; geração do ciclo em lote.
**Dep:** 87-42a, 87-41 · **Est:** G · **Executor:** @dev

#### 87-44 — `/platform` completo
...
- Todas as rotas passam por `withPlatformAdmin` e `platformQuery` (`87-42a`).
...
**Dep:** 87-43 · **Est:** G · **Executor:** @dev + @ux-design-expert

#### 87-45 — ~~Impersonation auditada~~ — **REMOVIDA por decisão de produto (D14, 2026-07-30)**

**O ID 87-45 fica permanentemente vago e NÃO deve ser reaproveitado.** Isto não é lacuna, esquecimento nem story esquecida no meio da numeração — é registro deliberado de uma remoção, para que ninguém reabra o assunto achando que caiu por descuido.
...
**Por quê:** o dono do produto decidiu que a Trifold não terá capacidade de "ver como o cliente". Suporte opera apenas com metadados e agregados via `PLATFORM_READABLE_TABLES` (`87-42a` entrega o mecanismo; `87-42b` fecha a lista).
...
**Nenhuma outra story dependia de 87-45.** Verificado: a única menção era em 87-14, sobre reaproveitamento do wrapper de escopo, e essa story permanece obrigatória por razão própria (o piso das 166 rotas service-role).

#### 87-46 — Cron `subscription-lifecycle` (nunca suspende)
...
- **Depende de 87-13** (convenção de path `{org_id}/…`): com os objetos sob o prefixo da org, a exclusão é enumerável e verificável por prefixo. Sem 87-13, não há como provar que sobrou zero objeto — por isso a dependência é dura, não conveniência.
...
**Dep:** 87-43, **87-13** · **Est:** M · **Executor:** @dev + @data-engineer
...
#### 87-47 — `org_integrations` + Vault + `resolveIntegration` com fallback explícito
...
**Dep:** 87-44 · **Est:** M · **Executor:** @dev + @data-engineer

#### 87-48 — Roteamento reverso de webhook por tenant (WhatsApp e Meta), com dual-run
...
- **Dual-run** antes do cutover: resolve pelos dois caminhos, usa o legado, loga divergência (mesmo padrão de 87-20).
...
**Dep:** 87-47, 87-24 · **Est:** G · **Executor:** @dev

#### 87-49 — Sienge e Resend por org + `/platform/orgs/[id]/integracoes`
...
- (O item "convite de usuário só cria usuário na org de quem convida" **saiu desta story e foi para 87-22**, Onda 2 — ver justificativa lá.)
**Dep:** 87-47 · **Est:** M · **Executor:** @dev
...
#### 87-50 — Provider de gateway contra a interface existente
...
PRE-0 (PR #308 em produção)
  └─ 87-1 ─ 87-2 ──┬─ [87-4 87-5 87-6 87-7] ─────────────────┐   (Lote 1 — paralelo)
                   ├─ 87-8 ─ 87-9 ─ 87-10 ───────────────────┤   (policies por domínio)
                   ├─ 87-11 ─ 87-12 ─ 87-13 ─────────────────┤   (Lote 2 — Storage)
                   ├─ 87-14 ─ 87-15 ────────────────────────┤   (Lote 3 — piso de código)
                   ├─ 87-16 ────────────────────────────────┤   (platform_admin / P4)
PRE-1 ─ 87-3 ──────┴─ 87-17 ──────────────────────────────────┴─ 87-18  ← gate bloqueante, baseline=0
                                                                   │
  87-19 ─ 87-20 ─ 87-21 ─ 87-22 ─ 87-23 ─ 87-24 ─ 87-25 ───────────┤   ONDA 2
                                                                   │
  87-26 ─ 87-27a ─┬─ 87-27b🔒 (preço) ─┐                          │
                  └─ 87-28 ─ 87-29 ─ 87-30 ─ 87-31 ─┴─ 87-32 ─────┤   ONDA 3 (🟢 vendável só com 27b)
                                                                   │
  87-33 ─ 87-34 ─ 87-35 ─ 87-36 ────────────────────────────────────┤   ONDA 4 (±5% × 14d)
                                                                   │
  87-37 ─ 87-38 ─ 87-39 ─ 87-40 ─ 87-41🔒 ────────────────────────┤   ONDA 5 → 💰 COBRÁVEL
                                                                   │
  87-42a ─┬─ 87-42b🔒 (lista/PEND-4)                              │
          └─ 87-43 ─ 87-44 ─ 87-46   (87-45 removida por D14) ────┤   ONDA 6
                                                                   │
  87-47 ─ 87-48 ─ 87-49  (sob demanda)    87-50  (sob demanda) ────┘   ONDAS 7-8
```

**Caminho crítico (a corrente mais longa, sem folga):**

```
87-1 → 87-2 ─┬─ 87-8 → 87-9 → 87-10 ──┐
   (PRE-0     ├─ 87-11 → 87-12 → 87-13 ┤   ← Storage: corrente longa, e 87-46 depende de 87-13
    trava      ├─ 87-14 → 87-15 ────────┤
    o 87-2)    └─ 87-17 (PRE-1) ────────┴→ 87-18   ← gate bloqueante, baseline = 0
   → 87-19 → 87-20 → 87-21 → 87-22 → 87-24 → 87-25
   → 87-26 → 87-27a → 87-28 → 87-31 → 87-32        ← fim da Onda 3
   → 87-33 → 87-34 → 87-36
   → 87-37 → 87-38 → 87-39 → 87-40                 ← enforcement de cota

Marcos comerciais, que NÃO estão no caminho crítico técnico:
  🟢 vendável  = fim da Onda 3 **+ 87-27b** (preço)  → travado por PEND-1 e PEND-1c
  💰 cobrável  = fim da Onda 5 **+ 87-41** (excedente) → travado por PEND-1b
```

As três correntes paralelas da Onda 1 (policies, Storage, piso de código) foram omitidas do caminho crítico anterior, assim como 87-27a. A corrente de Storage é a mais longa das três e é a que dita, na prática, quando 87-18 fecha.
...
**Correção importante sobre o Storage.** Uma versão anterior deste epic dizia que "o Storage não bloqueia 87-18", o que contradizia o próprio `Dep:` de 87-18 (**"87-4 … 87-17, todas"**). **Vale o `Dep:`: 87-11/12/13 estão dentro da Onda 1 e bloqueiam 87-18.** A frase anterior estava errada e era perigosa por uma razão concreta: **87-46 (exclusão pós-cancelamento, D13) depende duramente de 87-13** — sem a convenção de path `{org_id}/…`, a exclusão de Storage não é enumerável nem verificável por prefixo. Desescopar o Storage da Onda 1 com base naquela frase derrubaria silenciosamente a fundação da exclusão LGPD, quatro ondas depois e sem ninguém perceber.
...
|---|---|---|---|---|
| PRE-0 | Migration `209_hotfix_rls_org_scope.sql` (PR #308) **aplicada em produção** | **87-2** | tudo a partir de 87-2 | PR aberto em `hotfix/rls-org-scope-lote0`, QA PASS, **não aplicado**. **Não bloqueia 87-1** — criar a esteira de CI independe de migration aplicada |
| PRE-1 | Projeto Supabase descartável criado | 87-3, 87-17 | **87-18 e todas as ondas seguintes** — 87-18 depende de 87-17, e a Onda 2 inteira depende de 87-18. Sem o projeto, o epic **para no fim da Onda 1** | autorizado (D6), **não existe** |
| PRE-2 | Não existe CI nem husky | 87-1 é criação do zero | — | fato verificado |
| PRE-3 | Preço dos 3 tiers (PEND-1) + cota por tier (PEND-1c) + excedente (PEND-1b) | **87-27b, 87-41** | **apenas essas duas**, depois da quebra 27a/27b. Antes da quebra o fecho era de **20 stories** (87-27 → 87-32 → Ondas 4-8) | PEND-1, PEND-1b, PEND-1c |
| PRE-4 | Contrato de operador de dados (LGPD) | faturamento do 1º cliente | — | jurídico, fora do escopo técnico (CON-10) |
| PRE-5 | **PEND-4 respondida** | 87-42b | nenhuma story existente — mas a resposta pode **criar** uma story G de acesso pontual (§2.2). Precisa chegar **antes de a Onda 6 ser planejada**, não antes de começar | aberta |
...
- [ ] `lib/billing/**` e as tabelas do Epic 78 permanecem intocadas, exceto a re-ancoragem de policy em 87-16.
...
- [ ] `platform_audit_log` nasceu append-only **na mesma migration que a criou** (87-16), nunca existiu mutável.
...
Juntas, PEND-1 e PEND-1c travam **87-27b** e, com ela, o marco **"vendável"**. Não travam mais o resto da Onda 3: a quebra de 87-27 em **27a** (catálogo e composição, livre) e **27b** (preços e cotas, 🔒) tirou 18 stories do fecho transitivo desses dois números.

### Bloqueia a Onda 5 — e trava o marco "cobrável"

**PEND-1b — Preço do atendimento excedente.**
Preço por atendimento acima da cota, e **se há faixas decrescentes** (ex.: os primeiros N excedentes a X, os seguintes a Y). Sem isso `ai_overage_tiers` fica vazio, o excedente **não é faturável** (story 87-41) e a cobrança correta não fecha.

**Também de sua alçada, em 87-37: a regra de contagem de atendimento.**
...
A suspensão é manual (D5, fechado). Falta o limiar: quantos dias em `past_due` antes de o cron alertar a Trifold? Sugestão da arquitetura: 10 dias. Baixo impacto — 87-46 pode ser draftada com o número como campo a preencher.
...
|---|---|---|
| **PEND-1** — preço dos 3 tiers | `87-27b` | 🟢 vendável |
| **PEND-1c** — cota de atendimentos/mês por tier *(nova)* | `87-27b` | 🟢 vendável |
| **PEND-1b** — preço do excedente + faixas | `87-41` | 💰 cobrável |
| **Regra de contagem de atendimento** | `87-37` (não mais `87-33` — a Onda 4 anda sem ela) | 💰 cobrável |
| **PEND-4** — o que o super-admin vê | `87-42b`, e pode **criar** uma story G nova | planejamento da Onda 6 |
| **PEND-6** — dias em `past_due` | `87-46` (campo a preencher, não bloqueia draft) | — |
| **PEND-8** — existe trial? | `87-21` (parâmetro, não bloqueia draft) | — |
| **PEND-9** — nomes `lib/tenancy`/`lib/revenue` | nada; renomeação de ~30 arquivos se mudar depois | — |
| **PRE-1** — Supabase descartável | `87-3`, `87-17` → **e o epic inteiro a partir de 87-18** | todos |
| **PRE-0** — PR #308 em produção | `87-2` (**não** `87-1`) | todos |
...
| PEND-2 | Medir custo real por baixo da venda em atendimento? | **Sim** → D11 | 2026-07-30 | 87-33, 87-34, 87-35, 87-37 |
| PEND-7 | Política no 100% da cota | **`overage`** para `active`, `hard_stop` em trial, hard cap 3× → D12 | 2026-07-30 | 87-38 |
| PEND-5 | Retenção pós-cancelamento | **90 dias de export, depois exclusão definitiva** → D13 | 2026-07-30 | 87-46 |
| PEND-3 | Impersonation | **Não haverá** → D14 | 2026-07-30 | Removeu 87-45; ver §3.2 |

---

## 15. Handoff para o Story Manager (@sm)

"Desenvolver as stories detalhadas do Epic 87 (SaaS Multi-Tenant). Considerações-chave:

- **Este epic é o 86; as stories são `87-1` … `87-50`.** Os prefixos 84 e 85 pertencem a dois fixes pontuais não relacionados (`84-1-fix-created-by-auth-id-email`, `85-1-fix-subject-ausente-email-templates-api`) — não confundir nem sobrescrever.
- **As três stories com 🔒 são `87-27b`, `87-41` e `87-42b`.** Não draftar nenhuma delas: exigiriam inventar preço (PEND-1/PEND-1b), cota (PEND-1c) ou a lista de tabelas do `/platform` (PEND-4), e o Artigo IV proíbe. **`87-27a` e `87-42a` — as metades livres — estão liberadas**, e juntas destravam quase toda a Onda 3 e a Onda 6.
- **87-37 e 87-38 foram desbloqueadas** por D11 e D12. `87-37` ainda precisa da regra de contagem de atendimento antes de ir para `Ready` — mas **87-33 não precisa mais dela**: a Onda 4 inteira anda sem resposta do Gabriel, porque `consumed_atendimentos` saiu de 87-33 e nasce em 87-37.
- **Front de draft imediato: 27 stories.** Ondas 0, 1 e 2 inteiras (`87-1` … `87-25`, 25 stories) mais `87-27a` e `87-42a`. A Onda 2 ficou livre com a correção do ciclo O2↔O3 — ela não depende mais de nenhum artefato comercial. As demais stories não-🔒 (Ondas 3-8) também não têm furo de provenance nem cadeado; ficam gated por **ordem de entrega**, não por falta de informação.
- `87-1` pode começar **hoje** — não depende de PRE-0. `87-8`/`87-9`/`87-10` podem ser draftadas, mas **não promovidas a `Ready`** antes de 87-2 emitir `rls-gate-baseline.json`: a lista de tabelas de cada lote é a partição desse arquivo. `87-3` e `87-17` idem, até PRE-1 existir.
...
| Sua story **estende** um artefato incremental | **Declare a extensão na AC.** Vale especialmente para `PLATFORM_READABLE_TABLES` (regra de crescimento em `87-22`) e para `.github/workflows/ci.yml` (acrescentar job, nunca reescrever) |
| Sua story **quebra** em `a`/`b`/`c` | **Avise o @pm:** as linhas da §10.1 que apontam para o ID antigo precisam apontar para a sub-story certa. Aconteceu com `rls-gate-baseline.json`, que ficou dizendo `87-2` depois da quebra em `2a/2b/2c` |
...
- **`87-45` não existe** — removida por D14 (sem impersonation). O ID fica vago de propósito e não deve ser reaproveitado; a numeração salta de 87-44 para 87-46.
- **Ordem é lei:** nenhuma story de Onda 3+ entra antes de 87-18 (gate bloqueante e verde). Vender antes da Onda 1 é o único erro irrecuperável do epic.
- Migrations começam em **210** (verificado em 2026-08-03); a numeração 193 citada na arquitetura está **duas renumerações atrás**. São 222 arquivos `.sql` com 20 prefixos duplicados — ver §0.2, inclusive a armadilha de ordem lexicográfica que virou AC de `87-3`.
- Toda story que toca dado existente precisa de fase dual-run explícita nas AC — é o que substitui o staging que não existe.
- **Dois** arquivos exigem QA gate reforçado e teste do caminho de falha: `lib/permissions.ts` (87-28) e `packages/ai/src/client/anthropic.ts` (87-34). `lib/auth.ts` saiu da lista com D14 — sem overlay de impersonation, este epic não toca `getServerUser()`.
- **Toda story G deve ser quebrada no draft** pela regra do §10 (mais de uma fase expand→migrate→contract ⇒ mais de uma story; janela de observação ⇒ story própria). Candidatas já mapeadas: 87-2, 87-12, 87-13, 87-20, 87-44, 87-48.
- Reuso obrigatório (IDS: REUSE > ADAPT > CREATE): `MODULE_LABELS`/`MODULE_DESCRIPTIONS` em 87-29; `unstable_cache` de `permissions.ts` em 87-28; padrão de lembrete do Epic 78 em 87-39; padrão de RLS de `131_imobiliarias.sql` em 87-16.
