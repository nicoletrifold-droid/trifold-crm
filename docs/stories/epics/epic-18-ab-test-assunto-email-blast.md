...
stories_done: [80.1, 80.2, 80.3, 80.4, 80.5]
stories_next: []
---
...
- [x] Admin consegue ativar "Teste A/B de assunto" no Passo 2 do wizard e informar 2 versões de assunto
- [x] Ao confirmar o blast, a audiência é dividida automaticamente ~50/50 entre variante A e B (tolerância de arredondamento para número ímpar de destinatários)
- [x] Cada `email_log` enviado carrega qual variante recebeu (rastreável em consulta/relatório)
- [x] Tela de detalhe/lista do blast mostra, por variante: enviados, taxa de abertura, taxa de clique — lado a lado, sem indicar vencedor
- [x] Nenhuma regressão no envio de blasts sem A/B ativado (comportamento atual continua idêntico quando a feature não é usada)
- [x] Rate limiting (Story 78-1) continua sendo respeitado — envio de A/B não abre uma via paralela que ignore `daily_quota`

## Stories

| Story | Título | Executor | Quality Gate | Complexidade | Status |
|---|---|---|---|---|---|
| 80.1 | Schema: colunas de A/B em `email_blasts` + `variant` em `email_logs` | @data-engineer | @dev | P | Done |
| 80.2 | Wizard: toggle de teste A/B + campo de Assunto B no Passo 2 | @dev | @qa | M | Done |
| 80.3 | Split 50/50 + tagging de variante no enfileiramento do blast | @dev | @qa | M | Done |
| 80.4 | Agregação de métricas por variante (enviados, taxa de abertura, taxa de clique — sem vencedor automático) | @dev | @qa | P | Done |
| 80.5 | UI de resultados do teste A/B (números lado a lado por variante) | @dev | @qa | M | Done |
...
**ACs:**
- [x] Migration aplicada em dev antes de prod (seguir convenção do projeto de checar numeração antes de criar)
- [x] Colunas nullable/com default não quebram nenhuma query existente sobre `email_blasts`/`email_logs`
- [x] Índice em `email_logs(variant)` combinado com `triggered_by` se a query de agregação (Story 80.4) precisar (avaliar na implementação)
...
**ACs:**
- [x] Toggle desligado (padrão) preserva 100% o comportamento atual — nenhuma mudança visível pra quem não usa A/B
- [x] Toggle ligado exige os 2 campos preenchidos antes de avançar (mesma validação `canProceed` já existente, estendida)
- [x] Passo 3 (Confirmação) mostra resumo indicando que é um teste A/B e os 2 assuntos
...
**ACs:**
- [x] Split determinístico o suficiente para ser auditável (ex: ordenar por id do lead antes de dividir, não aleatório sem seed) — decisão de implementação, documentar no Dev Notes
- [x] Continua respeitando a fila/rate limiting existente (Story 78-1) — nenhum caminho de envio direto que pule `email_sends_queue`
- [x] Blast sem A/B ativado continua usando exatamente o fluxo atual (sem variant, sem split)
...
**ACs:**
- [x] Cálculo correto de taxas por variante, testável com dados de exemplo
- [x] Nenhuma lógica de comparação/decisão entre variantes — só agregação
...
**ACs:**
- [x] Blasts sem A/B continuam mostrando a UI atual sem alteração
- [x] Métricas exibidas batem com a agregação da Story 80.4
- [x] Nenhum texto ou badge de "vencedor"/"líder" — só os números lado a lado (Variante A vs Variante B)
...
Epic concluído: 5/5 stories Done, todos os Critérios de Sucesso atendidos, todos os quality gates PASS. Todas as story files têm evidência de teste manual (dados fabricados em produção, sempre limpos após validação). A decisão de produto "sem vencedor automático" foi respeitada de ponta a ponta (schema, agregação e UI) — nenhuma lógica de comparação/decisão entre variantes foi introduzida em nenhum ponto do sistema.
