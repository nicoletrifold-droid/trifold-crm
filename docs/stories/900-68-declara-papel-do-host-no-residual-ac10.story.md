...
- **Story:** 900-68 — número reconfirmado livre em 2026-09-04 (maior story existente em
  ...
  abertos nesta data).
- **Status:** Done (PR #572 squash-mergeado em `main` em 2026-09-04 — `66b083f5`; a CI de `main` nesse commit fechou `success`: `type-check · lint · test` SUCCESS e `gate de tenancy (não-bloqueante)` SUCCESS, run `33875271606`)
...
|---|---|---|---|
| 2026-09-04 | 1.1 | **Promovida a `Done` — PR #572 squash-mergeado em `main` (`66b083f5`).** A CI de `main` no commit de squash fechou **`success`** (run `33875271606`): o check bloqueante `type-check · lint · test` SUCCESS e `gate de tenancy (não-bloqueante)` SUCCESS — ou seja, o objetivo P0 da story (destravar a CI de todo o repositório) está medido no próprio `main`, não só na branch. Confirmado depois: o PR seguinte a entrar (#570, `3b809946`) também fechou CI `success` (run `33877335287`). Nenhuma alteração de código nesta virada — só a linha de Status e este registro. | Gage (@devops) |
