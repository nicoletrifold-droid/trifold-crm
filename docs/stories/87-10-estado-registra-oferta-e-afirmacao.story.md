...
| 2026-08-27 | v0.3 | **Nada nesta story muda — registro de fronteira, para que ninguém relitigue.** A `87-17` (defeito de produção de 26/08: a Nicole nega horário livre) precisava saber o que havia sido oferecido no turno anterior, e o @sm propôs implementar o **sítio nº 7** de `ofertas_do_sistema` dentro dela. **Arbitrado pelo @po em 27/08: NÃO.** A `87-17` **não escreve e não lê** `ofertas_do_sistema` nem `afirmado_pela_nicole`; ela resolve o caso por **recálculo no próprio turno**. Consequências para esta story, todas no sentido de "nenhuma": **(a)** a `AC1-(ii)` continua válida **ao número** — o `tsc` depois da remoção segue dando **exatamente os 2 erros** de `agenda-state.test.ts:48-49`, e a trava *"se aparecer um terceiro erro, PARE"* continua significando o que significava; **(b)** a premissa *"produção tem ZERO registros com os dois campos"* **não** precisa de remedição; **(c)** a garantia categórica *"nada lê o campo — nem para escolher o nível de um log"*, que decidiu o corte da `AC8`, fica **intacta**, e a leitura segue integralmente no `W3-2e` (Onda 3), como o cabeçalho manda. Parecer: `docs/qa/po-validation-87-17.md` §2. | @po (Pax) |
