...
**Status:** Done (@devops, 2026-09-03) — em produção desde 2026-09-03 (`dpl_EGqFvA9ENYBydWY3rTXgzJM3SgPw`), smoke test completo aprovado, PR #564 mergeado em `main` como `6cd6898c`
...
| 2026-09-03 | 1.3 | **Done.** PR #564 mergeado (squash) em `main` como `6cd6898c` após confirmação explícita do usuário. Antes do merge, os 11 achados do CodeRabbit foram triados um a um contra o código: **2 eram reais e foram corrigidos** no commit `f4574358` — (a) `build-dist.mjs` fazia `rm -rf` num `outRoot` vindo de `process.argv[2]`, então `node scripts/build-dist.mjs .` apagaria a pasta-fonte com os ~77 MB de `assets/`+`uploads/` fora do git (guarda adicionada; a sugestão literal do CodeRabbit deixava `/` passar e foi corrigida); (b) o arquivo do gate **não parseava como YAML** (escalar do QA-3 com `": "` em contexto de bloco abortava o documento inteiro) — resolvido só com aspas, texto do @qa intacto. Os outros 9 ficaram como follow-up documentado no PR, nenhum bloqueante. Verificado que as duas correções não mudam o artefato publicado (`--dry-run` reproduz 2112/2011/956/5951/2794). | @devops (Gage) |
...
**Fechamento (2026-09-03):** PR #564 mergeado em `main` como `6cd6898c` após confirmação explícita do usuário; conferido por comparação de conteúdo (em repo squash-merge `git log` não serve de prova) que `main` e a branch são idênticas nos paths da story. Status → **`Done`**. Como este projeto não é deployado por git, o merge é apenas registro — produção já estava com a 90-1 no ar desde o deploy.
