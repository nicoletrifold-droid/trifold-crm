# Story 86-3 — Projeto Supabase Descartável + Harness de Isolamento

## Metadata
- **Epic:** 86 — Trifold CRM → SaaS Multi-Tenant com Cobrança Modular
- **Onda:** 0 — Esteira e observabilidade (sem mudança funcional)
- **Story:** 86-3
- **Status:** Draft — **decisão deliberada do @po, não pendência de validação.** Validada com **GO condicional (8/10)** em `docs/qa/epic-86-po-validation.md`; permanece `Draft` porque a transição para `Ready` está amarrada à existência real de PRE-1 (não a mais revisão de conteúdo). Ver "Bloqueio explícito" abaixo — em especial o item 4, que resolve a ambiguidade entre "já validada" e "ainda em Draft".
...
**PRE-1 não existe ainda.** O epic (`depends_on`, frontmatter) registra: *"projeto Supabase descartável criado (autorizado pelo dono do produto, ainda não existe). Bloqueia 86-3 e 86-17."* A validação do @po (rodada 3) é ainda mais direta: *"PRE-1 — o projeto Supabase descartável. Sem ele o epic para no fim da Onda 1, e essa parada chega antes do que parece."*

Isso significa, concretamente:

1. Esta story **já foi draftada, revisada e validada pelo @po** — GO condicional (8/10), registrado em `docs/qa/epic-86-po-validation.md` e no Change Log desta story. A validação de conteúdo **está feita**; nada aqui depende de mais revisão.
...
- Wiring do job de isolamento no `.github/workflows/ci.yml` como job **bloqueante** — nesta onda, mesmo depois que `86-3` existir, o job de isolamento (quando os testes da Onda 1 existirem) roda contra este projeto descartável mas **não trava PR** até a Onda 1 provar maturidade (mesma lógica de catraca não-bloqueante de `86-2c`, mas para um job diferente — decisão explícita de story futura, não desta).
...
### Testing Standards
- Não há suíte automatizada que testa "o script de reset funciona" além de rodá-lo e inspecionar o resultado manualmente (mesmo padrão de infraestrutura observado em `86-1`). A validação de idempotência (AC4) é um passo manual documentado no Dev Agent Record, não um teste Vitest.
...
- **Depende de:** `86-1` (para que exista onde gravar secrets de CI e, futuramente, wire o job de isolamento — embora o wiring em si seja Onda 1)
...
- **Bloqueia diretamente:** testes cross-tenant da Onda 1 (`tests/tenancy/cross-tenant.spec.ts`, mencionados como critério de saída da Onda 1 no epic), e `86-17` (citado no `depends_on` do frontmatter do epic como também bloqueado por PRE-1)
