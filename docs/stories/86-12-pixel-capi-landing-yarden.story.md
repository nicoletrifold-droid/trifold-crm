...
| 2026-09-02 | 0.15 | **PR #553 mergeado em `main` por squash — commit de squash `86ea676aa7bbfb26de74f834c87993f342d21d7c`, `mergedAt: 2026-09-02T12:10:16Z`, **1 pai** (conferido por `git rev-list --parents`).** O SHA vem do campo `mergeCommit.oid` da API do GitHub, mas o objeto **não é um merge commit de verdade** — tem um pai só; a rodada 0.8 chamou o equivalente de "merge commit" e essa imprecisão fica registrada aqui em vez de reescrita no histórico dela. Traz para `main` o conteúdo/design definitivos da landing (rodada 0.9), o link da política de privacidade e o PDF (rodada 0.14). Antes do merge, o commit final do @dev (`66632608`, correção da contagem de assets no README, **1 arquivo / 1 linha**) foi pushado sem `--force` (fast-forward `6021a8c8..66632608`) — a contagem foi **reconferida por `git ls-files landing-pages/yarden/assets`: 13 arquivos**, não aceita do relato nem da sugestão do CodeRabbit (que dizia "10", errado porque o `.vercelignore` esconde `*.pdf` da visão dele). Estado re-lido imediatamente antes do merge: `state: OPEN`, `mergeable: MERGEABLE`, `mergeStateStatus: CLEAN`, `reviewDecision: APPROVED`, `headRefOid: 66632608`, e os **7** checks em `SUCCESS` (`type-check · lint · test`, gate de tenancy não-bloqueante, migrations não-bloqueante, CodeRabbit, Vercel Preview Comments, Vercel – trifold-crm, Vercel – trifold-crm-teste). Merge **sem `--admin`**, sem bypass. **Método por evidência, não por default:** os 3 merges mais recentes de `main` (#556 `1260f107`, #552 `b6812146`, #551 `b7a1b64b`) têm **1 pai cada** e `allow_squash_merge: true` — squash é o padrão real. **O `APPROVED` do CodeRabbit NÃO cobria o commit final, e isso foi tratado em vez de ignorado:** a review `APPROVED` foi submetida às `11:51:48Z` e o commit `66632608` só nasceu às `11:56:19Z`; o que de fato validou o commit final foi a review incremental sobre ele, cujo status `CodeRabbit` foi criado às `11:59:49Z`. Aguardado até `12:08:42Z` (≈11 min, polling de reviews, review comments e issue comments com filtro por `created_at`): **zero** achado novo, zero comentário, nenhuma reabertura. Registro explícito porque check verde do CodeRabbit significa "rodou", não "aprovou" — a prova aqui é a ausência de comentário na review do head commit, não a cor do check. **Título do PR corrigido de propósito antes de mergear:** dizia "(3 formulários, **12** assets)" e o repo tem `squash_merge_commit_title: COMMIT_OR_PR_TITLE`, logo a contagem errada — exatamente o defeito que `66632608` acabara de consertar — seria gravada na mensagem do commit de `main`. Passou a "13 assets". **Conteúdo conferido em `main` depois do merge, por leitura da árvore e não por presunção:** `git ls-tree -r main -- landing-pages/yarden/assets` devolve **13** arquivos (incluindo `politica-de-privacidade.pdf`) e `git show main:landing-pages/yarden/README.md` na linha 26 diz "13 arquivos: 5 imagens (jpg + webp cada) + 2 logos SVG + 1 PDF". `main` local atualizada por `git fetch origin main:main` (`f3b9d504..86ea676a`), **sem `checkout`** — a working tree principal tem ~60 arquivos em voo de outros agentes e está com a branch v1 no HEAD. **Branch remota `feat/86-12-yarden-conteudo-definitivo-v2` deletada** (o repo tem `delete_branch_on_merge: false`, é passo manual), e só depois de provar absorção **por diff de árvore, não por `git log`**: `git diff --name-only 66632608 origin/main` difere em **8 arquivos, todos do PR #556** (`packages/web/src/app/api/webhook/whatsapp/*` + memórias de agente), e **zero** nos paths da 86-12 (`landing-pages/yarden/`, story, gate, decision-log) — em repo squash-merge `git log origin/main..branch` nunca sai vazio e não prova nada. **A branch remota `feat/86-12-yarden-conteudo-definitivo` (v1, `b4e7c8d3`) NÃO foi deletada, de propósito:** carrega os commits do Microsoft Clarity (`8cf64d90`, `b4e7c8d3`) de um agente concorrente, deliberadamente excluídos deste PR na rodada 0.13, e é o HEAD ativo da working tree principal. Worktree de trabalho (`scratchpad/wt`) removida após o merge confirmado. **Armadilha deixada registrada porque vai morder quem commitar a branch v1:** a cópia desta story **na working tree principal está desatualizada** — é a versão pré-PR (sem as rodadas 0.13/0.14, ainda afirmando "12 arquivos" no File List), conferida por `sha256` divergente e por `git diff main` (5 inserções / 15 remoções, com `main` tendo *mais* conteúdo). Por isso esta entrada **não foi escrita ali**, e sim na branch `docs/86-12-pr-553-merged` cortada de `origin/main`; quem for commitar a v1 precisa **não** deixar a cópia velha atropelar a de `main`. **Status mantido em `InReview` — merge não é `Done`, e a autorização do stakeholder (lucas@trifold.eng.br) cobria o merge e só o merge.** Nenhum projeto Vercel criado, nenhum deploy disparado, nenhuma env var gravada. **T12/T13 seguem `[ ]`**: o projeto Vercel `yarden` continua não existindo, `trifold.eng.br/yarden/` **não está no ar** e o **AC13 segue não validado**. Consequência colateral a não confundir com defeito: os 4 links de política (3 checkboxes + rodapé) e o PDF estão em `main` mas **não servidos em lugar nenhum**, porque a página inteira ainda não foi publicada — o `86.12-QA-002` está corrigido **no código** e segue **formalmente `OPEN` no registro de QA** — a tabela de achados (linha ~1223, `low`) e o Gate Status (linha ~1341) ainda o marcam `OPEN`, e o único lugar que o declara fechado é o Dev Agent Record (~1118), alçada do @dev. Ou seja: **defeito resolvido, gate não re-verificado.** O fechamento formal é do @qa, como a própria rodada 0.14 já sinalizou, e **não** foi feito nesta rodada — nada aqui deve ser lido como o gate de QA tendo sido fechado por @devops. O merge publica automaticamente apenas a fatia `packages/web` (que nesta rodada não mudou — o discriminador multi-landing já subiu no #512). | @devops (Gage) |
| 2026-09-02 | 0.16 | **Dois ajustes pontuais de conteúdo na landing, os dois por decisão do stakeholder (lucas@trifold.eng.br): rótulo do hero "EM BREVE" → "LANÇAMENTO" e upgrade da imagem do mapa de localização.** Escopo deliberadamente mínimo — **nenhuma linha de CSS de layout, nenhum teste, nenhum comportamento, nenhum arquivo fora de `landing-pages/yarden/` e desta story** foi tocado. **(1) Rótulo do hero.** `index.html:499`: `<span class="hero-eyebrow">EM BREVE</span>` → `LANÇAMENTO`. Motivo do negócio registrado: o empreendimento **saiu da fase de pré-lançamento e está em lançamento**, então o rótulo antigo passou a ser informação falsa na página. Mesmo elemento, mesma classe, mesma posição, mesmo estilo — **só o nó de texto**. É a **segunda divergência autorizada em relação ao mockup** "Yarden LP v1.png" (a primeira foi o texto do checkbox, rodada 0.11), que segue escrevendo "EM BREVE" nesse ponto; por isso os **dois comentários de CSS que citavam a string antiga foram reescritos, não deixados apodrecer**: o de `--fonte-serif` (~234) dizia *"«EM BREVE» é serifado no mockup (...) são 8 glifos decorativos"* — a contagem virou falsa com 10 glifos, então passou a descrever "o rótulo acima do wordmark" sem fixar o texto; e o de `.hero-brand` (~273) dizia *`marca "EM BREVE \| yarden"`*, e agora registra explicitamente que o mockup diz "EM BREVE", que o publicado é "LANÇAMENTO" por decisão do stakeholder, e que **geometria e estilo seguem o mockup — só o texto divergiu** (sem isso, uma comparação futura página↔mockup leria a diferença como defeito, e um `grep "EM BREVE"` acharia só CSS sem achar a marcação). **Não quebra o layout, e isso foi MEDIDO, não presumido** — o rótulo tem 2 glifos a mais e a caixa é de largura fixa (`.hero-brand{width:28.7%}`, 56% no mobile): por `boundingBox` no Chromium, desktop 1440px dá `eyebrow` **393,9px** dentro de `brand` **413,3px** e mobile 390px dá **207,2px** dentro de **218,4px**, com altura de **uma linha** nos dois (24,86px / 13,30px) — ou seja **não há wrap nem overflow**, e o cedilha/tilde renderizam na serifada do sistema. **(2) Imagem do mapa** (seção `.split--invista`, "Invista no novo centro urbano de Maringá"). Fonte nova entregue pelo stakeholder: `Book virtual_Página_05.jpg` (2640×1485) do book do empreendimento — **primeiro asset da página que NÃO vem do mockup**. Ganho real: a arte antiga representava o prédio como **silhueta marrom plana** e trazia o título manuscrito "Gleba Itororó"; a nova traz **render real do edifício** (torre branca com vegetação nas sacadas) e já embute, na própria imagem, o título "Localização" e o endereço "Rua Carlos Meneghetti, 168 / Gleba Itororó". Usada **inteira**, sem recriar nem combinar com a antiga. **Aspect ratio resolvido sem cortar nada de conteúdo, e sem tocar no container** — o caminho (1) da instrução, não o (2): a fonte é 16:9 (1,778) contra 1,413 da antiga, mas os 1,778 **não são a arte** — são a arte mais **duas tarjas brancas laterais** de página de book. Detectadas por varredura de pixel (limiar <245 por canal, perfil por coluna/linha), com **borda dura**: conteúdo em `x 270..2370`, `y 1..1484`, ou seja **2101×1484 = 1,4157** — praticamente o mesmo 1,4129 (900×637) do asset antigo. Removida tarja branca não é recorte de conteúdo: **os 6 pins com rótulo (Catedral, Bosque II, Av. JK, `yarden`, Parque Do Ingá, Av. Itororó, todos com a linha-guia), o título "Localização" e as duas linhas de endereço estão íntegros** — conferido olhando o arquivo processado, não deduzido do número. Consequência: **zero mudança de CSS** — `.split img{width:100%;height:auto}` não impõe `aspect-ratio` nem `object-fit`, então a caixa acompanha a proporção intrínseca; a seção só fica ~1px mais baixa. **Reescala com a mesma disciplina de performance dos outros assets** (resize para o tamanho exibido, não os 2640px): 900px de largura, que é o teto real de exibição — não o desktop (o `<img>` mede **641,4px** a 1440px de viewport, porque a coluna é `635fr` de `.split--invista`), e sim a faixa de **coluna única** abaixo de 980px, onde a imagem chega a ~940px. Altura derivada da proporção: **900×636** (`900 × 1484 / 2101 = 635,7`), 1px menos que os 637 antigos. Atributos `width`/`height` do `<img>` atualizados de `637` para `636` para **não** ficarem desalinhados do arquivo — conferido em runtime que `naturalWidth`/`naturalHeight` (900/636) batem com `attrW`/`attrH`, logo não há CLS por dimensão mentida nem distorção (proporção exibida 641,4/453,3 = 1,4151 = a intrínseca). **Orçamento de bytes deliberadamente igualado ao do asset antigo**, para um upgrade visual não virar regressão de peso: jpg `q84` progressivo **129.183 B** (antigo 131.969) e webp `q82` **85.970 B** (antigo 86.768) — os dois **menores** que os anteriores, apesar da fonte melhor. **Arquivos sobrescritos, nomes mantidos** (`assets/mapa-gleba-itororo.jpg`/`.webp`): é o mesmo papel na página, e manter o nome faz o teste bidirecional de integridade de `assets/` da rodada 0.9 continuar valendo sem alteração — contagem segue **13 arquivos**, zero órfão, zero referência quebrada. **`alt` ajustado por necessidade, não por gosto:** dizia "Vista aérea de Maringá com a localização do Yarden na Gleba Itororó e pontos de referência (...)", o que descrevia a silhueta; passou a "com o render do Yarden na Rua Carlos Meneghetti, 168, Gleba Itororó" — o endereço agora **está desenhado na imagem** e texto embutido em imagem é invisível para leitor de tela, então deixar o `alt` velho esconderia informação que o visitante vidente enxerga. Lista de pontos de referência preservada. **Confirmação VISUAL, não só numérica** (a página foi servida localmente e fotografada com Chromium em `1440×900` e `390×844`, `deviceScaleFactor: 2`): hero mostra "LANÇAMENTO" serifado em uma linha acima do wordmark `yarden`, e a seção do mapa mostra a imagem inteira, sem corte, com os 6 pins, "Localização" e o endereço legíveis nos dois breakpoints; **0 erro de console** (`console.error` + `pageerror`) e `currentSrc` resolvendo para o **`.webp`** (o `<source>` do `<picture>` continua vencendo). Porta 8123 usada de propósito: havia servidor de outro agente na 8080 e derrubá-lo não era necessário. **Validações — paridade com baseline medida NA MESMA ÁRVORE, porque o trabalho foi feito em `git worktree`:** `npx vitest run landing-pages/yarden` **2 arquivos / 50 passed** (inclui o teste bidirecional de `assets/`, que passa porque os nomes não mudaram). Suíte completa `npx vitest run`: **71 failed / 4266 passed + 6 expected fail** — e o `diff` da lista de falhas **antes** (`git stash`) contra **depois** é **vazio**, logo **0 regressão**; as falhas são o artefato conhecido de worktree (`node_modules` linkado da checkout principal faz `@trifold/ai`/`@trifold/shared` resolverem para a branch de lá — ex. `Cannot find package '@trifold/ai/src/flows/loop-breaker'`), em 3 arquivos de `packages/web`, nenhum tocado aqui. Mesmo método nos outros dois gates, saídas comparadas por `diff` antes/depois: `npm run type-check` **idêntico** (7/8 tasks, as mesmas 14 linhas de erro de resolução `@trifold/*`, nenhuma em `landing-pages/`) e `npm run lint` **idêntico** (8/8 tasks, **0 errors** / 33 warnings pré-existentes de `packages/web`). Número cru sem baseline não seria prova de nada. Nota de ambiente reconfirmada: `type-check` precisa de `NODE_OPTIONS=--max-old-space-size=8192`. **Nada de infra:** `vind-residence` e `trifold-design-system` **não tocados**, nenhum teste novo (não há comportamento novo a travar), nenhum checkbox de task alterado. **T12/T13 seguem `[ ]` e o AC13 segue não validado** — o projeto Vercel `yarden` continua não existindo e `trifold.eng.br/yarden/` **não está no ar**, então estes dois ajustes estão em código e **não** em produção. **Status NÃO alterado (`InReview`).** Branch nova `fix/86-12-yarden-lancamento-mapa` cortada de `origin/main` atualizado (as branches antigas da story já foram mergeadas/limpas), commit local com pathspec explícito, **sem `git push`** — alçada do @devops. | @dev (Dex) |
| 2026-09-02 | 0.17 | **Título do formulário do hero passa de "Antecipe-se ao lançamento" para "Cadastre-se para mais informações", por decisão do stakeholder (lucas@trifold.eng.br).** Motivo do negócio: o rótulo do hero virou "LANÇAMENTO" na rodada 0.16 justamente porque o empreendimento **saiu do pré-lançamento** — e um título que convida a "antecipar-se ao lançamento" ficou dizendo o contrário da própria página; o novo texto é um CTA neutro, que não datava o estágio comercial. **Escopo confirmado explicitamente com o stakeholder e respeitado ao pé da letra: só o formulário do hero.** O hero tem **duas marcações do MESMO formulário**, alternadas por media query em 980px (`hero-form--sobreposto` com `id="leadForm"`, desktop; `hero-form--estatico` com `id="leadFormMobile"`, mobile/tablet) — conferido lendo o HTML, não presumido —, então o título estava **duplicado** e os dois foram trocados, com texto idêntico, senão a página passaria a dizer coisas diferentes acima e abaixo de 980px. **O 3º formulário (`leadFormSaber`, seção "Quer saber mais?") NÃO foi tocado:** segue com o próprio `<h2>Quer saber mais?</h2>` e o próprio `<p class="sub">Deixe seu contato e receba informações exclusivas</p>` — confirmado por screenshot da seção nos dois breakpoints, não só pelo diff. **Diff total: `1 file changed, 2 insertions(+), 2 deletions(-)`, as duas nos nós de texto dos `<h2>`.** Mesma tag, mesmas classes, mesma posição, **zero linha de CSS**, zero teste, zero comportamento, zero arquivo fora de `landing-pages/yarden/index.html` e desta story. Nenhum comentário de CSS/HTML citava a string antiga (`grep` "Antecipe" devolvia **exatamente as 2 linhas de marcação**, nada mais no repositório), então não houve comentário a reescrever — diferente da rodada 0.16, onde havia. **A instrução previa ajustar `clamp()` se o texto mais longo quebrasse a caixa; a medição mostrou que NÃO quebra, então o CSS ficou intacto — não-mudança medida, não presumida.** O `.hero-form h2` tem `text-transform:uppercase`, logo o que pinta é "CADASTRE-SE PARA MAIS INFORMAÇÕES" (33 caracteres contra 25), e a caixa é de largura amarrada ao viewport (`width:max(23.0874%,268px)` no desktop, full-width no mobile) com `font-size:clamp(12.5px,1.025vw,17px)` no card e `1.6em` no título. **Varredura de 1px em 320–2560px (2241 larguras), com `antes` e `depois` medidos na MESMA sessão de Chromium para o `diff` valer:** o `<h2>` **nunca chega a 3 linhas** (só 1 ou 2 em toda a faixa) e **não há overflow em nenhuma largura** — nem do próprio `<h2>` (`scrollWidth` ≤ largura da caixa) nem do documento (`documentElement.scrollWidth` == `clientWidth`). Altura do card **idêntica ao baseline em toda a faixa exceto na banda 430–546px**, onde o título antigo cabia em 1 linha e o novo pede 2: ali o card cresce **+28,1px** (467,2 contra 439,1) — e como abaixo de 980px o card é **bloco em fluxo normal**, isso só empurra o conteúdo seguinte, sem sobrepor nada e sem estourar nada (conferido por screenshot a 480px). Em 390px e 320px **não há delta nenhum**: os dois títulos já ocupavam 2 linhas. Acima de ~2089px o antigo passa a caber em 1 linha e o novo segue em 2, sem mudança de altura do card (o `min-height` domina). A folga mínima medida é **0,1px a 1719px**, e isso **não é fragilidade**: é exatamente o ponto de virada do wrap, onde a quebra deixa de ser "CADASTRE-SE PARA / MAIS INFORMAÇÕES" e passa a "CADASTRE-SE PARA MAIS / INFORMAÇÕES" — o navegador só adota a linha mais larga quando ela cabe, por definição. O piso real de segurança é a palavra mais longa e inquebrável, "INFORMAÇÕES": no card mais estreito de todos (268px a 980px de viewport, `contentW` 238px, fonte 20px) ela mede ~143px, com folga de sobra. **Trap de fonte que quase invalidou a medição, e que ficou registrado porque não é óbvio:** o `<link>` do Google Fonts usa `display=optional`, e `font-display:optional` **nunca faz swap** — numa carga fria a página inteira pinta na fallback (`-apple-system`), que é **~6% mais estreita** que Montserrat. Medir assim daria um falso "cabe". As medições válidas foram feitas interceptando o CSS do `fonts.googleapis.com` e reescrevendo para `font-display:block`, com `document.fonts.ready` + reload, e a prova de que Montserrat de fato pintou é a divergência `wMont` **309px** vs `wFallback` **291,7px** para o mesmo texto (na primeira tentativa os dois batiam, denunciando a fallback). **Confirmação VISUAL nos dois breakpoints pedidos, com Montserrat pintada** (`deviceScaleFactor: 2`): a **1440px** o card do hero mostra "CADASTRE-SE PARA / MAIS INFORMAÇÕES" centralizado em 2 linhas equilibradas, dentro da caixa marrom, com `Ç` e `Õ` inteiros (cedilha e tilde sem corte), sem tocar as bordas nem os campos abaixo, e o hero inteiro segue coerente ("LANÇAMENTO | yarden" à esquerda, campos Nome/DDD+Celular/E-mail, checkbox de política, botão "Cadastre-se agora!" e o aviso de SPAM, todos intactos); a **390px** mostra "CADASTRE-SE PARA MAIS / INFORMAÇÕES", também 2 linhas centralizadas com folga lateral; a **480px** (a única banda com delta de altura) o card acomoda as 2 linhas sem corte. A seção "Quer saber mais?" fotografada nos dois breakpoints continua com "QUER SABER MAIS?" + o subtítulo e o botão "Enviar agora mesmo" — **intocada**. Único elemento sobreposto nas fotos é o botão flutuante do WhatsApp cobrindo parte do aviso de SPAM no mobile, que é `position:fixed` e **pré-existente** (aparece igual no baseline) — não introduzido aqui. **Validações — paridade com baseline medida NA MESMA ÁRVORE, porque o trabalho é em `git worktree` (`node_modules` é symlink da checkout principal):** `npx vitest run landing-pages/yarden` **2 arquivos / 50 passed** — nenhum teste afirmava sobre o título, então a troca não exigiu ajuste de teste nem escondeu quebra. Suíte completa `npx vitest run`: **71 failed / 4266 passed + 6 expected fail** em 3 arquivos de `packages/web`, e o `diff` da lista de falhas **antes** (`git stash` do arquivo) contra **depois** é **vazio** → **0 regressão**; as falhas são o artefato conhecido de worktree (`@trifold/ai`/`@trifold/shared` resolvendo pela branch da checkout principal, ex. `Cannot find package '@trifold/ai/src/flows/loop-breaker'`), nenhum dos 3 arquivos tocado aqui. Mesmo método nos outros dois gates: `NODE_OPTIONS=--max-old-space-size=8192 npm run type-check` → **12 linhas de erro antes e as mesmas 12 depois, `diff` vazio**, todas de resolução `@trifold/*`, nenhuma em `landing-pages/`; `npm run lint` → **0 errors / 30 warnings** pré-existentes de `packages/web`, nenhuma no arquivo alterado (8/8 tasks em cache, o que por si só confirma que `landing-pages/` não entra no grafo do turbo — `landing-pages/yarden/package.json` não declara script nenhum, então esses dois gates **não cobrem** este arquivo e a cobertura real dele é o `vitest` + a medição no browser). **T12/T13 seguem `[ ]` e o AC13 segue não validado** — o projeto Vercel `yarden` continua não existindo e `trifold.eng.br/yarden/` **não está no ar**, então este ajuste está em código e **não** em produção. **Status NÃO alterado (`InReview`).** Trabalho na worktree/branch já existentes `fix/86-12-yarden-lancamento-mapa` (nenhuma branch ou worktree nova criada), commit local com pathspec explícito, **sem `git push`** — alçada do @devops. | @dev (Dex) |
...
- `landing-pages/yarden/assets/` — **13 arquivos**: 10 recortes do mockup "Yarden
  LP v1.png" (rodada 0.9), 2 recortes do book do empreendimento (o mapa de
  localização, substituído na rodada 0.16) + o PDF da política de privacidade
  (rodada 0.14).
  ...
  - `mapa-gleba-itororo.jpg` / `mapa-gleba-itororo.webp` (900×636; recorte de
    `Book virtual_Página_05.jpg` do book do empreendimento, **não** do mockup —
    substituídos na rodada 0.16, mesmo nome/papel na página)
