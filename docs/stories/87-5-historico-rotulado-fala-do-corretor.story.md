...
- **Deploy A**, 24 h, responsável nomeado (Marcos ou Thielly): (i) **um caso de reativação
  EXERCITADO, não esperado** — ver a caixa; (ii) **a régua da contradição**, reescrita abaixo;
  (iii) `M1` e `M4` medidos **pela rotina da Story 87-3**, sem aumento.
  ...
  **AC12-(ii) — a régua da contradição.** *(reescrita por @po em 15/08, achado A3 do gate. A redação
  anterior — "zero ocorrências de valor do corretor repetido, por busca textual" — está refutada por
  medição em (ii-c).)*

  **O denominador, declarado, e ele NÃO é "as respostas da Nicole":** a população é o **turno da
  Nicole cujo histórico já continha fala de corretor na mesma conversa** — é a única população que
  esta story muda. Medida por @po contra produção em 15/08, últimos 30 dias: **588 turnos da Nicole
  no total, dos quais apenas 34 (em 13 conversas) estão nessa população — 1,13/dia**. E a
  distribuição é **em rajada, não contínua: 9 dos últimos 15 dias tiveram ZERO**; um dia teve 6.
  **A janela de 24 h tem ~60 % de chance de produzir ZERO turno observável.**

  - **(ii-a) O que FECHA a AC é o turno provocado da (i), classificado.** Na resposta da Nicole ao
    turno provocado, uma de três saídas, colada literal e nomeada: **ATRIBUI** (nomeia o corretor
    como autor do valor) · **CONTRADIZ** (opõe a regra geral — *"entrada mínima é 20 %"*, *"em torno
    de 80 mil"* — ao valor que o corretor já fechou com o lead) · **DESMENTE** (diz ao lead que a
    mensagem *"não era pra você / foi engano"*). Verde **só** em ATRIBUI. CONTRADIZ ou DESMENTE é
    **gatilho de reversão, sem discussão**. Referência do experimento: `HEAD` CONTRADIZ 17/20;
    rótulo sem instrução DESMENTE 12/20; com a instrução, ATRIBUI 10/10.
  - **(ii-b) A leitura observacional é por CLASSIFICAÇÃO, não por busca textual, e tem piso de n.**
    Ler **todos** os turnos da população acima ocorridos na janela — todos, não amostra — e
    classificar cada um nas mesmas três saídas. **Se n < 5 o resultado é INCONCLUSIVO, jamais "sem
    regressão".** Critério de extensão, escrito antes: estender a janela até **n ≥ 5** ou até **7
    dias corridos**, o que vier primeiro; chegando aos 7 dias com n < 5, fechar como **INCONCLUSIVO
    POR BASE RATE** e registrar que a evidência da story é o turno provocado (ii-a) mais a suíte —
    não a janela. Consulta da população (roda hoje, sem código novo):
    ```sql
    select m.id, m.conversation_id, m.created_at, m.content
    from messages m
    where m.role = 'assistant' and m.created_at > '<hora do deploy A>'
      and exists (select 1 from messages b
                  where b.conversation_id = m.conversation_id
                    and b.role = 'broker' and b.created_at < m.created_at)
    order by m.created_at;
    ```
  - **(ii-c) 🔴 O que NÃO serve de régua — medido por @po em 15/08, não opinado.**
    - **A busca textual da redação anterior já está saturada em zero.** Dos 34 turnos da população,
      **0 repetem valor de corretor**; o único com cifra (*"seu orçamento de R$ 400 mil"*, 05/08)
      repete o orçamento **do próprio lead**. Uma régua que mede 0 antes vai medir 0 depois — e
      alguém leria isso como sucesso.
    - **A "regra comercial interna" (entrada mínima 20 % / 80 mil) é PIOR ainda, e não vira AC.**
      ...
      reais"*) e `qualification.ts:34-36`. Contar essa frase é contar a Nicole **obedecendo**.
      (2) Na população-alvo ela vale **0 de 34** hoje. (3) As **4** ocorrências dos últimos 30 dias
      estão **todas na população oposta** — turnos **anteriores** a qualquer fala de corretor (07/08
      ×2, 12/08, e a de 28/07 que ecoa o *"dando 80k de entrada"* dito pelo **próprio lead**) — e a
      story **não muda nenhuma das quatro**. Precisão da régua para o defeito-alvo: **0 de 4**.
      O `7/10 → 0/10` do experimento é real, mas é **condicional à fixture**, que é o pior caso
      construído; em produção esse denominador vale 34 turnos por mês, e nele a frase não aparece.
  - **(ii-d) Controle positivo, obrigatório e provocado.** Provar que o defeito some mede metade.
    Segunda conversa de teste, **sem fala de corretor**, o lead perguntando valor/condições: a Nicole
    **deve continuar** entregando a resposta prescrita (*"entrada mínima de 20 %"*, *"em torno de 80
    mil"*). Emudecer nesse cenário é a story ter quebrado comportamento legítimo — e **também é
    reversão**. Referência de produção: essa resposta saiu **4 vezes em 30 dias**, todas na população
    sem corretor.
...
- a Nicole se reapresentando ("Sou a Nicole…") a lead que já vinha conversando com o corretor —
  ⚠️ **gatilho de INSPEÇÃO, não de reversão automática**: baseline medido por @po em 15/08 (90 dias)
  é **3 em 40 turnos da população-alvo, 7,5 %**. Já acontece hoje; reverter na primeira ocorrência
  seria reverter por defeito **herdado**. Vira reversão se vier junto de contradição de valor, ou se
  a taxa da janela ficar acima do baseline com **n ≥ 5**;
- ~~o rótulo `[CORRETOR HUMANO` aparecendo em qualquer mensagem enviada ao lead~~ →
  **este gatilho é inalcançável e foi substituído. Ver a caixa abaixo.**

> 🔴 **[@po 15/08 — achado A4 do gate] NÃO HÁ gatilho automático para o vazamento do rótulo. Isto
> está declarado, não fingido.** Três medições minhas contra produção:
>
> 1. **A higienização apagou o gatilho como ele estava escrito.** `stripSystemBlocks` roda **antes**
>    de persistir e de enviar (`pipeline.ts:1181-1186`), e `saveMessages` grava `assistantMessage` —
>    o texto **já limpo** (`pipeline.ts:1625`). O rótulo não pode aparecer nem na mensagem entregue
>    ao lead nem na tabela `messages`. Conferido: `messages` com `[CORRETOR HUMANO` = **0**, e vai
>    continuar 0 haja o que houver. Um gatilho que não pode disparar não é gatilho.
> 2. **O evento que sobra nunca disparou — nenhuma vez, em toda a história.** `system_events` com
>    `event_type = 'NICOLE_SYSTEM_BLOCK_LEAK'`: **0 registros no total**, contra 471 `CLAUDE_RESPONSE`
>    em 30 dias (ou seja: o caminho de emissão funciona; o fenômeno é que não ocorre). E o **único
>    vazamento real de marcação que existe em produção não gerou evento**: a mensagem
>    `3a885e0e-bc31-4f8a-a0f6-13fc159bde38`, de **06/08 10:04 UTC**, foi entregue ao lead com o bloco
>    `[SISTEMA: horário 11h do sábado 08/08 — LIVRE]`. Ela é **anterior em ~6 h** ao deploy da 75-279
>    (`fa100677`, 06/08 13:05 UTC) — não é falha da guarda, é a **base rate**: **1 em 1.320** turnos
>    da Nicole em toda a história, e isso com a exposição do `[SISTEMA`, que é muito maior que a do
>    rótulo do corretor (só ~1 turno/dia tem corretor no histórico). Valor esperado numa janela de
>    24 h: **~0,02**. **"Zero ocorrências" é o resultado esperado inclusive num mundo quebrado.**
> 3. **Distinguir as duas marcações NÃO exige campo novo — e por isso o `marcacao:` no metadata fica
>    FORA desta fase** (é caminho de decisão novo, e o `event_type` é o que a instrumentação lê). O
>    `metadata.raw_message` já guarda os **500 primeiros caracteres da saída crua do modelo**, antes
>    da limpeza. A consulta abaixo roda hoje, sem uma linha de código — eu a executei contra produção
>    (retorna 0 linhas, como esperado pelo item 2):
>    ```sql
>    select id, created_at, metadata->>'lead_id' as lead_id, metadata->>'raw_message' as cru
>    from system_events
>    where event_type = 'NICOLE_SYSTEM_BLOCK_LEAK'
>      and created_at > '<hora do deploy A>'
>      and metadata->>'raw_message' ilike '%[CORRETOR HUMANO%';
>    ```
>    **Limite declarado:** se a marcação cair depois do caractere 500 da saída crua, a consulta não a
>    enxerga — nesse caso o evento entra como **"marcação indeterminada"** e vai para inspeção
>    manual, junto com o `raw_message`. É o mínimo defensável sem tocar em código.
>
> **O substituto — quem olha, o quê e quando. Isto é o gatilho; não há automático:**
> - **Quem:** o **responsável nomeado da AC12** (Marcos ou Thielly). Sem nome, o deploy não sai (D7),
>   e esta caixa **não supre o nome**.
> - **O quê 1 —** rodar a consulta do item 3 ao fim da janela. Qualquer linha com `[CORRETOR HUMANO`
>   = **reversão imediata**. Zero linhas **não é evidência de sucesso**, pela base rate acima.
> - **O quê 2 —** ler, um a um, **todos** os turnos da população da **AC12-(ii-b)**. É o único lugar
>   onde o defeito pode aparecer dentro da janela. Se n < 5, o veredito é **INCONCLUSIVO**.
> - **O quê 3 —** a reapresentação (bullet acima), contra o baseline de 7,5 %.
> - **Quando:** ao fim das 24 h e, se a janela for estendida por n < 5 (AC12-ii-b), de novo no
>   fechamento da extensão.
