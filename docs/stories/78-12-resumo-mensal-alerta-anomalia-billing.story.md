...
| 2026-07-14 | 1.0 | **Deploy em produção @devops (Gage) — Status InReview → Done.** Migration renumerada 170→171 (170 consumida por Story 80-1 em origin/main) e aplicada em prod (`dsopqkqjkmhytudaaolv`) via Management API (201, smoke-test colunas/tabelas/RLS/policies OK, reexecução idempotente). Gates: lint/type-check 0 erros novos, 46/46 testes. PR #191 (squash, SHA 59619753). Deploy git de produção READY (crm.trifold.eng.br). Dry-runs: summary 200 (skipped not_day_1), anomaly 200 (2 serviços, 0 enviados). | @devops (Gage) |
