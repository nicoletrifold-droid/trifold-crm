...
- [x] AC1: Botão "Gerenciar Tipos" adicionado na página `/dashboard/brindes`, ao lado do botão "Gerenciar Datas" existente
- [x] AC2: Clicar em "Gerenciar Tipos" abre modal `TiposModal` com lista de tipos cadastrados (nome, tamanho, cor, badge ativo/inativo)
- [x] AC3: Modal exibe formulário inline para criar novo tipo com campos: Nome (obrigatório), Descrição (opcional), Tamanho (opcional, ex: "P", "M", "G", "GG" ou texto livre), Cor (opcional)
- [x] AC4: Ao salvar novo tipo, chama `POST /api/brindes/tipos` e atualiza a lista sem reload
- [x] AC5: Cada tipo na lista tem botão "Editar" que abre sub-form inline com os campos preenchidos; salvar chama `PATCH /api/brindes/tipos/[id]`
- [x] AC6: Cada tipo tem botão "Desativar/Ativar" que chama `PATCH /api/brindes/tipos/[id]` com `{ ativo: !tipo.ativo }`; tipos inativos aparecem com visual desabilitado (opacidade reduzida)
- [x] AC7: Tipos inativos não aparecem no seletor de brinde ao registrar entrega (AC10)

### Seleção de Tipo no Registro de Entrega
- [x] AC8: O dropdown de status de entrega (no `status-badge.tsx`) exibe, além das opções de status, um seletor de tipo de brinde quando uma data comemorativa está selecionada
- [x] AC9: O seletor de tipo lista apenas os tipos `ativo = true` da org, ordenados por nome
- [x] AC10: Ao confirmar a entrega com status "Entregue", o `brinde_tipo_id` selecionado (ou null se não selecionado) é enviado no body do `POST /api/brindes/entregas`
- [x] AC11: O tipo de brinde selecionado é persistido e exibido como tooltip ou texto secundário no badge de status após salvar

### Exibição na Tabela
- [x] AC12: A coluna "STATUS" na tabela de brindes exibe o tipo de brinde abaixo do badge de status quando `brinde_tipo_id` estiver preenchido (texto secundário pequeno: nome do tipo + tamanho + cor)
- [x] AC13: Se não há tipo associado à entrega, a coluna exibe apenas o badge de status (sem alteração no layout atual)

### Carregamento de Dados
- [x] AC14: A lista de tipos ativos é carregada uma vez na inicialização da página (`GET /api/brindes/tipos?ativo=true`) e passada como prop para os componentes que precisam (StatusBadge e TiposModal)
- [x] AC15: Após criar/editar/desativar um tipo no modal, `router.refresh()` atualiza os dados da página
...
- [x] Task 1: Adicionar interface `BrindeTipo` ao arquivo `types.ts` existente (ou criar se necessário)
- [x] Task 2: Criar `tipos-modal.tsx` com CRUD de tipos (criar, editar inline, ativar/desativar) (AC2, AC3, AC4, AC5, AC6)
- [x] Task 3: Atualizar `page.tsx` — carregar tipos via `GET /api/brindes/tipos?ativo=true`, adicionar botão "Gerenciar Tipos" e passar tipos como prop (AC1, AC14, AC15)
- [x] Task 4: Atualizar `status-badge.tsx` — adicionar seletor de tipo de brinde no dropdown de status, enviar `brinde_tipo_id` no body do POST entregas (AC8, AC9, AC10, AC11)
- [x] Task 5: Atualizar `brindes-table.tsx` — exibir nome/tamanho/cor do tipo abaixo do badge de status quando disponível (AC12, AC13)
- [x] Task 6: Verificar `npm run typecheck` e `npm run lint` sem erros — 0 erros, 6 warnings pré-existentes
- [x] Task 7: Testar no browser — fluxo completo: criar tipo → selecionar ao registrar entrega → verificar exibição na tabela
...
- `packages/web/src/app/dashboard/brindes/_components/types.ts` — modificado
