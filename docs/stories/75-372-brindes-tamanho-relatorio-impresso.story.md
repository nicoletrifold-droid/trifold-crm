...
**Status:** Done (PR #570 squash-mergeado em `main` em 2026-09-04 — `3b809946`, CI `success` no commit de squash, run `33877335287`; gate CONCERNS com SEC-001 aceito como dívida — fechado depois pela Story 75-373, gate PASS) · sem migration
...
| 04/09/2026 | @devops (Gage) | **Promovida a `Done` — PR #570 squash-mergeado em `main` (`3b809946`).** CI de `main` no commit de squash: **`success`** (run `33877335287`). Das duas pendências não-bloqueantes que o gate CONCERNS deixou abertas, a nº 2 está **fechada**: a story de follow-up do `escapeHtml()` existe, é a **75-373** (gate **PASS**, SEC-001 fechado nos 9 sítios, PR #573) — este PR de registro não a mergeia. Segue aberta apenas **TEST-001** (LOW, teste do ramo `!t.nome` de `buildBrinde`), que é backlog e nunca foi bloqueante. Nenhuma alteração de código nesta virada — só a linha de Status e este registro. |
