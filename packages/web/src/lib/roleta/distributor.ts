...
export async function distributeLeadToNextBroker(
  ...
  if (cfg.priorizar_lead_ativo && lead.phone) {
    ...
    if (existingLead?.assigned_broker_id) {
      ...
      if (brokerRow) {
        ...
        // Story 75-247/75-249 — a visita futura é de quem atende o lead: a
        // Nicole pode ter agendado antes de o lead ter dono. Carimba e avisa.
  ...
  // Story 75-247/75-249 — visita agendada pela Nicole antes da distribuição fica
  // sem corretor; ao ganhar dono, a visita vai com o lead (e ele é avisado).
  await syncFutureVisitsWithLeadOwner({
