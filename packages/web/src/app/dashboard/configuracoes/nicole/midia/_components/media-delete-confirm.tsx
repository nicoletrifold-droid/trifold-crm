...
export function MediaDeleteConfirm({ assetId, assetTitle, base }: Props) {
  ...
  async function handleDelete() {
    setDeleting(true)
    const res = await fetch(`/api/nicole/media?id=${assetId}`, { method: "DELETE" })
    setDeleting(false)
    if (res.ok) {
      router.push(base)
      router.refresh()
  ...
  return (
    <div className="rounded-lg border border-red-200 bg-red-50 p-5 dark:border-red-500/30 dark:bg-red-500/10">
      ...
      <p className="mb-4 text-sm text-red-600 dark:text-red-400">
        Tem certeza que deseja excluir <strong>{assetTitle}</strong>?{" "}
