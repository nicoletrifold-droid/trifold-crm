...
export function CorretorDetail({
  brokerId,
  canEdit,
}: {
  brokerId: string
  ...
  const loadBroker = useCallback(() => {
    return fetch(`/api/brokers/${brokerId}`)
      .then((r) => r.json())
      .then((d) => {
        setBroker(d.data)
  ...
  useEffect(() => {
    loadBroker().finally(() => setLoading(false))
    fetch("/api/properties")
      .then((r) => r.json())
      .then((d) => setProperties(d.data ?? []))
      .catch(() => {})
    fetch(`/api/brokers/${brokerId}/assignments`)
      .then((r) => r.json())
  ...
  const assignedNames = properties
    .filter((p) => assignedProperties.includes(p.id))
    .map((p) => p.name)
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-stone-900 dark:text-stone-100">
            {editing ? "Editar Corretor" : broker.user.name}
          </h1>
          <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">{broker.user.email}</p>
        </div>
        {canEdit && !editing && (
          <button
            onClick={() => { setEditing(true); setMessage("") }}
            className="flex shrink-0 items-center gap-2 rounded-lg bg-orange-600 px-4 py-2 text-sm font-medium text-white hover:bg-orange-700"
          >
      ...
      {!editing ? (
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-stone-900 dark:ring-1 dark:ring-stone-800">
          <dl className="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
            <Field label="E-mail" value={broker.user.email} />
            <Field label="Telefone / WhatsApp" value={broker.user.phone || "—"} />
            <Field label="CRECI" value={broker.creci || "—"} />
            <Field label="Tipo" value={TYPE_LABELS[broker.type] ?? broker.type} />
            <Field label="Limite de leads" value={`${broker.max_leads ?? 50} leads`} />
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-stone-400 dark:text-stone-500">Status</dt>
              <dd className="mt-1.5 flex flex-wrap items-center gap-2">
                ...
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                  broker.is_available ? "bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "bg-stone-100 text-stone-600 dark:bg-stone-700/50 dark:text-stone-300"
                }`}>
            ...
            <div className="sm:col-span-2">
              <dt className="text-xs font-medium uppercase tracking-wide text-stone-400 dark:text-stone-500">Empreendimentos vinculados</dt>
              <dd className="mt-1.5 flex flex-wrap gap-2">
                {assignedNames.length > 0 ? (
                  assignedNames.map((name) => (
                    <span key={name} className="rounded-full bg-orange-50 px-2.5 py-0.5 text-xs font-medium text-orange-700 dark:bg-orange-500/15 dark:text-orange-300">
                      {name}
                    </span>
                  ))
                ) : (
                  <span className="text-sm text-stone-400 dark:text-stone-500">Nenhum</span>
                )}
              </dd>
            </div>
          </dl>
          ...
          {message && (
            <p className="mt-5 text-sm text-green-600 dark:text-green-300">{message}</p>
          )}
        </div>
      ) : (
        /* ===================== MODO EDIÇÃO ===================== */
          ...
          <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-stone-900 dark:ring-1 dark:ring-stone-800">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => { setEditing(false); setMessage(""); setPhone(broker.user.phone ?? "") }}
                  className="rounded-lg border border-stone-200 px-5 py-2.5 text-sm font-medium text-stone-600 hover:bg-stone-50 dark:border-stone-700 dark:text-stone-400 dark:hover:bg-stone-800"
                >
                  Cancelar
                </button>
              </div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <dt className="text-xs font-medium uppercase tracking-wide text-stone-400 dark:text-stone-500">{label}</dt>
      <dd className="mt-1.5 text-sm text-stone-900 dark:text-stone-100">{value}</dd>
    </div>
  )
}
