...
import type { BoardCard } from "./lancamento-board"

interface Comment { id: string; body: string; created_at: string; author: string }

export function LancamentoCardModal({ card, onClose, onUpdated, onDeleted }: {
  card: BoardCard
  onClose: () => void
  ...
  async function saveField(patch: Partial<BoardCard>) {
    await fetch(`/api/lancamentos/cards/${card.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) }).catch(() => {})
