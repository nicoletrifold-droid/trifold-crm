-- 156_roleta_pick_no_perdido.sql
-- Story 75-118 — Perdido é terminal para a automação: a roleta não pode
-- (re)atribuir corretor a um lead que está na etapa "Perdido".
--
-- CONTEXTO: mark-lost põe o lead em Perdido mas deixa is_active=true e não limpa
-- assigned_broker_id. Um lead perdido que ficou sem corretor casava no cron
-- roleta-retry (que só olhava assigned_broker_id) e o distributor reatribuía +
-- reescrevia stage_id='novo' ("Aguardando atendimento") — ressuscitando o lead
-- descartado sem nenhuma ação de usuário (activity stage_change com user_id=null).
--
-- FIX (defensivo — o distributor.ts e o roleta-retry já guardam por stage_id antes
-- de chamar esta função): roleta_pick_and_advance passa a ignorar leads na etapa
-- Perdido ('00000000-0000-0000-0001-000000000008'), tanto no guard inicial quanto
-- no UPDATE final. Único jeito de um lead sair de Perdido continua sendo ação
-- manual de um usuário (mudança de etapa via UI/kanban).
-- Demais comportamento idêntico à 130 (guard de bolsão, round-robin, teto,
-- empreendimento, atômico).
